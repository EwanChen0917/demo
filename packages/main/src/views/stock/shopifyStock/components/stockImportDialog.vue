...
<script setup lang="ts">
  ...
  withDefaults(
    ...
    }>(),
    {
      requirementList: () => [
        '必填字段：',
        'MSKU、站点、跟踪状态、最大允许超售值、缺货售卖、库存同步',
        '校验规则：',
        '1、MSKU + 站点必须存在；',
        '2、MSKU+站点的数据不允许重复；',
      ],
      redirectPath: '/salesForecastingLog',
    }
  );
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = (await uploadOptions.value?.uploadService({
        ...
        importScene: uploadOptions.value.importScene,
        projectCode: uploadOptions.value.projectCode,
      })) as ImportResult;
