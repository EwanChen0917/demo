<template>
  ...
  >
      ...
      <el-input v-model="search[searchType]" clearable placeholder="请输入" style="width: 350px">
        <template #prepend>
          <el-select v-model="searchType" style="width: 120px">
            <el-option
              v-for="item in searchTypes.centerSearchTypes"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
    ...
    <template #buttons>
      <ExportBtn
        :service="erpApi.luteosErpPlanInventorySelfExportList"
        :params="{
          ...search,
        }"
      >
    ...
    >
      ...
      <el-table-column v-for="col in centerTableColumns" :key="col.prop" v-bind="col">
        <template #header>
          {{ col.label }}
          <el-tooltip placement="top" v-if="col?.tips">
            <template #content>
              <component :is="col?.tips['center']" />
            </template>
...
<script setup lang="ts" name="planCenterStock">
import { ref } from 'vue';
  import { ErpApi, erpApi } from '@/api';
  import useList from '@/hooks/list/useList';
  import WarehouseSelect from '../../components/warehouseSelect.vue';
  import { searchTypes } from '../../components/searchTypes';
  import { centerTableColumns } from '../components/tableColumns.tsx';
  ...
  const {
    ...
    ErpApi.LuteosErpPlanInventorySelfQueryList.RequestQuery,
    ErpApi.LuteosErpPlanInventorySelfQueryList.ResponseBody
  >({
    searchDefaults: {
      skuCode: undefined,
    ...
    service: erpApi.luteosErpPlanInventorySelfQueryList,
  });
