<template>
  ...
  >
      ...
      <el-input v-model="search[searchType]" clearable placeholder="请输入" style="width: 350px">
        <template #prepend>
          <el-select v-model="searchType" style="width: 120px">
            <el-option
              v-for="item in searchTypes.tikTokSearchTypes"
              :key="item.value"
              :label="item.label"
              :value="item.value"
            />
    ...
    <template #buttons>
      <ExportBtn
        :service="erpApi.luteosErpPlanInventoryTiktokExportList"
        :params="{
          ...search,
        }"
      >
...
<script setup lang="ts" name="planTiktokStock">
  import { ref } from 'vue';
  ...
  >({
    ...
    service: erpApi.luteosErpPlanInventoryTiktokQueryList,
  });
