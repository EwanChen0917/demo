    ...
    <main class="inventory-turnover-main">
      ...
      <div class="body" v-loading="loading">
        <el-scrollbar class="left left-scrollbar">
          <div>
            <!-- 趋势分析 -->
            <TrendAnalysis v-if="state.trendAnalysis" :trendAnalysis="state.trendAnalysis!" />
            <!-- 多维分析 -->
            <DrillDownAnalysis
...
<script lang="ts" setup>
  ...
  const state = ref<BiContracts.InventoryTurnoverResp>({});

  const loading = ref(false);
  const initData = async () => {
    loading.value = true;
    biApi.biInventoryQueryEnums(search).then((res) => {
      if (res) {
        dict.value = res;
      }
    });
    const res = await biApi.biInventoryQueryInventoryTurnover(search).finally(() => {
      loading.value = false;
    });
