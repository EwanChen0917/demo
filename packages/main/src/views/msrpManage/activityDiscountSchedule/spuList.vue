<template>
  ...
  >
    <template #search>
      <el-input v-model="search.spuCode" clearable placeholder="产品SPU" class="me-2">
        <template #prefix>
          <SvgIconSearch />
        </template>
      </el-input>
      <el-input v-model="search.skuCode" clearable placeholder="产品SKU">
        <template #prefix>
          <SvgIconSearch />
        </template>
      </el-input>
    ...
    <template #buttons>
      <el-button
        ...
        @click="addDialogRef?.open('新增SKU', null)"
      >
        新增SKU
      </el-button>
      <el-button
              ...
              :actions="[
                {
                  ...
                  type: 'primary',
                  row,
                  permissionCode: 'activityDiscountScheduleAdd',
                },
              ]"
              @click="handleActions"
            />
...
<script setup lang="ts" name="activityDiscountSchedule">
  ...
  const month = ref([
    dayjs(new Date()).add(0, 'month').format('YYYY-MM'),
    dayjs(new Date()).add(2, 'month').format('YYYY-MM'),
  ]);
  ...
  const {
    ...
    ProductApi.LuteosProductPromotionPlanListBySpu.RequestBody,
    ProductApi.LuteosProductPromotionPlanListBySpu.ResponseBody
  >({
    searchDefaults: {
      approveStatus: undefined,
      channel: undefined,
      countryCode: undefined,
      endDate: getEndDate(dayjs(new Date()).add(2, 'month').format('YYYY-MM')),
      gtm: undefined,
      skuCode: undefined,
      spuCode: undefined,
      startDate: dayjs(new Date()).add(0, 'month').format('YYYY-MM') + '-01',
    },
    // eslint-disable-next-line no-shadow
    pageSize: PAGE_SIZE,
    service: async (params) => {
      const res = await productApi.luteosProductPromotionPlanListBySpu(params);
      rowIndex.value = 0;
      mergeRules.value = [];
      planCode.value = [];
  ...
  const changeMonth = () => {
    if (dayjs(month.value[0]).add(2, 'month').isBefore(dayjs(month.value[1]))) {
      ElMessage.warning('时间跨度不能超过3个月');
      return;
    }
    search.startDate = month.value[0] + '-01';
    search.endDate = getEndDate(month.value[1]);
  };
  ...
  const handleActions = async (item) => {
    const { key, row } = item;
    if (key === 'add') {
      addDialogRef.value.open('添加活动', row);
    }
  };
