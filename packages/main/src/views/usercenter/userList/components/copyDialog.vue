<template>
  <el-dialog width="65%" title="复制权限" v-model="visible" @close="visible = false" align-center>
    <KeenList
      @refresh="refreshList"
...
<script setup lang="ts">
  ...
  const save = async () => {
    try {
      ...
      await memberApi.luteosMemberChangeUserRole({
        accountCodeList: selectedRows.value?.map((item) => item.accountCode),
        saveRoleCodeList: curRoleList.value,
        addRoleCodeList: [],
        deleteRoleCodeList: [],
      });
