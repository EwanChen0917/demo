    ...
    >
      ...
      <template v-if="false">
        ...
        <section v-show="visible" class="report-drawer-wrapper">
          <ReportDrawer branch="supplyChainStockingAnalysis" />
        </section>
      </template>
    </Multipane>
  </div>
...
<script setup lang="ts" name="supplyChainStockingAnalysis">
import { marketingApi, platformApi } from '@/api';
  ...
  const getIframeSrc = async () => {
    const res = await marketingApi.luteosMarketingBiReportStockUpUrl();
    if (res?.url) {
      loading.value = true;
