  ...
  <div class="report-container">
    ...
    >
      ...
      <section v-show="visible" class="report-drawer-wrapper">
        <ReportDrawer branch="amazonReport" />
      </section>
    </Multipane>
  </div>
</template>

<script setup lang="ts" name="amazonReport">
import { marketingApi, platformApi } from '@/api';
  ...
  //     dictCodes: ['marketing_amazon_belong'],
  //   });
  //   options.value = res?.dictMap?.marketing_amazon_belong || [];
  // };
  ...
  const getIframeSrc = async () => {
    const res = await marketingApi.luteosMarketingWeekReportAmazonBiUrl();
    if (res?.url) {
      loading.value = true;
