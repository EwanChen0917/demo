<template>
  <div class="mpc-new-product-analysis" :style="{ height: height }">
    <div class="main-container">
      <header class="header">
        <div class="header-left">
          <span style="padding-right: 8px">汇报对象</span>
          <el-cascader
            class="job-number-select"
        ...
        <div class="header-right">
          <el-button type="primary" v-if="getPermission({ type: 'spu' })" @click="openSPUAnalysis">
            新增SPU分析
          </el-button>
        </div>
      </header>
        ...
        <aside class="sidebar" :style="{ display: sidebarVisible ? 'block' : 'none' }">
          ...
          <div class="analysis-panel">
            <LtAnalysisReport
              ...
              :report-type="WeeklyReportType.MpcWeeklyReport"
              dateType="month"
              :branch="form.branch"
    ...
    <!-- SPU分析弹窗 -->
    <SpuDialog ref="spuDialogRef" @reload="handlerReload" />
  </div>
</template>

<script setup lang="ts" name="mpcNewProductAnalysis">
  import { MarketingContracts, mpcWeeklyReportApi } from '@/api';
  ...
  import SpuDialog from './spuDialog.vue';
  ...
  const analysisReportRef = ref(null);

  const getPermission = ({ type, data }) => {
    if (type === 'spu') {
      return true; // 本次迭代，无限制
    }
    if (type === 'add') {
      const gtm = gtmList.value.find((item) => item.value === form.branch);
      // 1 查看 2 编辑
      if (gtm?.type === 2) {
        return true;
      }
  ...
  const getOptions = async () => {
    const res = await mpcWeeklyReportApi.luteosMarketWeekReportQueryOptions({
      platform: 'lute_erp',
      componentName: 'mpcReportOptions', // 每个功能模块的标识
    });
  ...
  const iframeSrc = ref(
    '' // https://finebi.luteos.com/webroot/decision/v5/design/report/e7b370954b8e44239fe66b9e5b937ff9/view?entryType=7'
  );
  ...
  const handlerReload = () => {
    setIframeSrc(iframeSrc.value);
  };

  const spuDialogRef = ref<InstanceType<typeof SpuDialog>>();
  const openSPUAnalysis = () => {
    if (!form.branch) {
      ElMessage.warning('请选择品线');
      return;
    }
    spuDialogRef.value?.open();
  };
