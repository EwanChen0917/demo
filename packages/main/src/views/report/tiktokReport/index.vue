  ...
  <div class="report-container">
    ...
    >
      ...
      <section v-show="visible" class="report-drawer-wrapper">
        <ReportDrawer branch="tiktokReport" />
      </section>
    </Multipane>
  </div>
</template>

<script setup lang="ts" name="tiktokReport">
import { marketingApi, platformApi } from '@/api';
  ...
  //     dictCodes: ['marketing_tiktok_belong'],
  //   });
  //   options.value = res?.dictMap?.marketing_tiktok_belong || [];
  // };
  ...
  const getIframeSrc = async () => {
    const res = await marketingApi.luteosMarketingWeekReportTiktokBiUrl();
    if (res?.url) {
      loading.value = true;
  ...
  const isPermission = computed<boolean>(() => {
    return store.authList.includes('tiktokReport');
    // return true;
  });
  const handleLoad = () => {
    loading.value = false;
