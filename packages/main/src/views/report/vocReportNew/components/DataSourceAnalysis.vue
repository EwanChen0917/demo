<template>
  <div class="data-source-analysis">
    <div class="data-source-analysis__title">VOC数据来源分布情况 Top 5</div>
          ...
          >
            <template #default="scope">
              ...
              >
                ...
                ></div>
                <span>{{ scope.row.dataSource || '未知' }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column
...
<script setup lang="ts">
  ...
  const data = computed(() => {
    ...
    const [top1, top2, top3, top4, top5, ...rest] = detailData.value.dataSourcePropList || [];

    const topDataSource = [top1, top2, top3, top4, top5].filter(Boolean);
    if (topDataSource.length < 5) {
  ...
  const initEcharts = (el: HTMLElement) => {
    ...
    const option = {
      ...
      legend: {
        show: false,
        data: data.value.map((item: any) => item.dataSource),
