...
<script setup lang="ts">
  ...
  const serviceMap = {
    label: {
      template: erpApi.luteosErpBaseB2BClientSkuLabelDownTemplate,
      upload: erpApi.luteosErpBaseB2BClientUpload,
    },
    batchUpdate: {
      template: erpApi.luteosErpBaseB2BClientInfoDownTemplate,
      upload: erpApi.luteosErpBaseB2BClientUploadInfo,
    },
    quotation: {
      template: erpApi.luteosErpQuotationDownTemplate,
      upload: erpApi.luteosErpQuotationUpload,
    },
  };
  ...
  const downloadTemplate = async () => {
    ...
    const res = await serviceMap[props.type]
      .template({ ...props.downloadParams, actionType: props.actionType })
      .finally(() => {
