<template>
  <KeenCard v-if="!isAdd && form.baseInfo.orderStatus !== 101 && form.authNodeList?.length">
  ...
  >
    ...
    <!--    <KeenCard title="订单信息">
      ...
      >
        <el-row :gutter="20">
          ...
          &lt;!&ndash;          <el-col :md="8" :sm="12" :offset="0">
        ...
        <el-row :gutter="20">
          ...
          <el-col :md="8" :sm="12" :offset="0">
            <el-form-item label="邮箱" prop="receiveInfo.email">
              <el-input v-model="form.receiveInfo.email" :maxlength="32" />
            </el-form-item>
          </el-col>
          <el-col :md="8" :sm="12" :offset="0">
    ...
    <KeenCard title="产品信息">
      <!--      <el-space>
        ...
        <el-select v-model="unitCode" @change="handleUnitCodeChange" size="small">
    ...
    <KeenCard title="物流信息">
      ...
      >
        <el-row :gutter="20">
          <el-col :md="8" :sm="12" :offset="0">
            <el-form-item label="发货仓库" prop="baseInfo.warehouseCode">
              <el-select-v2
          ...
          <el-col :md="8" :sm="12" :offset="0">
            <el-form-item label="物流商" prop="baseInfo.spCode">
              <el-select-v2
                v-model="form.baseInfo.spCode"
                filterable
                clearable
                :options="spList"
                :props="{
          ...
          <el-col :md="8" :sm="12" :offset="0">
            <el-form-item label="物流渠道" prop="baseInfo.shippingCode">
              <el-select-v2
          ...
          <el-col :md="8" :sm="12" :offset="0">
            <el-form-item label="物流单号">
              <span>{{ form.baseInfo.orderCode }}</span>
            </el-form-item>
          </el-col>
          <el-col :md="8" :sm="12" :offset="0">
...
<script setup lang="ts" name="shopifyOrderAudit">
  ...
  const rules = reactive<FormRules>({
    ...
    'baseInfo.warehouseCode': [{ required: true, message: '请选择仓库' }],
    'baseInfo.spCode': [{ required: true, message: '请选择物流商' }],
    'baseInfo.shippingCode': [{ required: true, message: '请选择物流渠道' }],
  });
  ...
  const queryWarehouseShipList = async () => {
    const res = await erpApi.luteosErpOrderShipmentQueryWarehouseShipList({});
  ...
  const handleWarehouseChange = (val) => {
    ...
    spList.value = warehouseList.value.find((item) => item.warehouseCode === val)?.spInfos || [];
    shippingList.value = [];
  };
  ...
  const handleSpChange = (val) => {
    form.value.baseInfo.shippingCode = '';
    shippingList.value = spList.value.find((item) => item.spCode === val)?.shipInfos || [];
  };

  if (route.query.code) {
    Promise.all([initData(), queryWarehouseShipList()]).then(() => {
      spList.value =
        warehouseList.value.find(
          (item) => item.warehouseCode === form.value?.baseInfo?.warehouseCode
        )?.spInfos || [];
      shippingList.value =
        spList.value.find((item) => item.spCode === form.value?.baseInfo?.spCode)?.shipInfos || [];
    });
  }
  ...
  const save = async () => {
    ...
    const valid = await Promise.all([
      ...
      logisticsFormRef.value?.validate().catch(() => false),
    ]);
    if (valid) {
      if (!form.value.itemInfoList.length) {
        ...
        closeTabAndRefresh('shopifyOrder');
        router.push({
          name: 'shopifyOrder',
        });
      }
