...
/** itemSearchType 为供应链 SKU / MSKU 时，itemSingleSearchKey 需为空或不少于该长度才触发列表查询 */
const SHOPIFY_ITEM_SINGLE_SEARCH_MIN_LEN = 5;
const SHOPIFY_ITEM_SEARCH_SKU_MSKU_TYPES = [1, 2];
...
export default function useShopifyOrderList<
...
>({
  ...
  if (watchSearchKeys?.length) {
    ...
    const itemSingleArr = watchSearchKeys.filter(
      (item) => item === 'itemSearchType' || item === 'itemSingleSearchKey'
    );
    ...
    if (
      ...
      if (itemSingleArr.length > 0) {
        watch(
          itemSingleArr.map((key) => () => (search as any)[key]),
          () => {
            const type = Number((search as any).itemSearchType);
            const raw = (search as any).itemSingleSearchKey;
            const s =
              raw === undefined || raw === null || raw === ''
                ? ''
                : String(raw).trim();
            const isSkuOrMsku = SHOPIFY_ITEM_SEARCH_SKU_MSKU_TYPES.includes(type);
            if (isSkuOrMsku) {
              if (s.length === 0 || s.length >= SHOPIFY_ITEM_SINGLE_SEARCH_MIN_LEN) {
                debouncedSearchQuery();
              }
            } else {
              debouncedSearchQuery();
      ...
      const simpleWatchKeys = watchSearchKeys.filter(
        (k) => k !== 'itemSearchType' && k !== 'itemSingleSearchKey'
      );
      watch(
        simpleWatchKeys.map((key) => () => (search as any)[key]),
        () => debouncedSearchQuery()
      );
    }
  }
