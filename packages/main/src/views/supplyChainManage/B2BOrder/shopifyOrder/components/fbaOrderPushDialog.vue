    ...
    >
      <div class="fba-content">
        <div class="fba-content-left">
          ...
          <!-- <el-form-item label="详细地址3" prop="address3">
            <el-input v-model="form.address3" clearable maxlength="255"  />
          </el-form-item> -->
        </div>
            ...
            <div class="item-list" :class="{ 'has-scroll': form.items.length > 2 }">
              <div v-for="item in form.items" class="item-box">
                <div class="item-box-left flex">
                  <el-image class="item-box-left-image" :src="item?.productImageUrl || ''" />
                </div>
...
<script setup lang="ts">
  ...
  const dataInit = (data) => {
    ...
    if (data.itemBeanList?.length > 0) {
      const items = data.itemBeanList.map((item) => {
        return {
          count: item.numInt || 0,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await omsApi.omsFulfillmentOrderTransferFba({
        ...params,
      });
