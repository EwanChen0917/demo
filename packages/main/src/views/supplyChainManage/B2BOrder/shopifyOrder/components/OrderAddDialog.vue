...
<script setup lang="ts" name="shopifyOrderAdd">
  import { omsApi } from '@/api';
  import * as swal from '@/utils/swal';
  import { ElMessage } from 'element-plus';
  // import { Plus } from '@element-plus/icons-vue';
  // import GcSkuDialog from '@/views/supplyChainManage/efficiencyTools/shopifyOrder/components/GcSkuDialog.vue';
  ...
  const disabledStartDate = (time) => {
    // console.log(
    //   'disabledStartDate',
    //   form.value.buyerDeliveryEndTime,
    //   time.getDate() > new Date(form.value.buyerDeliveryEndTime).getDate()
    // );
    if (
      form.value.buyerDeliveryEndTime != null &&
      form.value.buyerDeliveryEndTime !== undefined &&
      form.value.buyerDeliveryEndTime !== ''
    ) {
  ...
  const queryPlaceOrder = async () => {
    const res = await omsApi.omsFulfillmentOrderQueryPlaceOrder({
      erpCode: code,
    });
  ...
  const submit = async () => {
    ...
    try {
      ...
      await omsApi.omsFulfillmentOrderPlaceOrder({
        erpCode: code,
        ...form.value,
      });
      ElMessage.success('下单成功');
      closeTabAndRefresh('shopifyOrder');
      router.push('/FulfillmentOrder');
      emit('success');
      close();
    } finally {
