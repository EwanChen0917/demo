<template>
  ...
  >
    <template #filters>
      ...
      <el-select
        style="width: 160px"
        v-model="search.markShippedStatus"
        placeholder="标发状态"
        filterable
        clearable
        collapse-tags
    ...
    <template #more-filter>
      ...
      <el-form-item label="禁止处理">
        <el-select
          ref="suspendStatusSelectRef"
          v-model="search.suspendStatus"
          placeholder="请选择"
          filterable
          clearable
          :teleported="false"
      ...
      <el-form-item label="订单标签">
        <el-select
          v-model="search.orderLabelCodeList"
          placeholder="请选择"
          filterable
          multiple
          clearable
    ...
    <template #custom-slot>
      <el-dropdown
        class="el-button-group--horizontal"
        popper-class="shopify-save-view-dropdown"
        split-button
        trigger="click"
      ...
      >
        <span class="d-flex align-items-center gap-1">
          ...
          />
          <span class="condition-name">{{ currentView?.conditionName || '保存视图' }}</span>
        </span>
        <template #dropdown>
          <el-dropdown-menu v-loading="viewLoading">
            <el-dropdown-item
              v-for="item in viewList"
              :key="item.conditionCode"
              :command="item.conditionCode"
            >
              <div
                class="view-save-dropdown-row d-flex align-items-center gap-2 pd-2"
                style="width: 100%"
              >
                <div class="d-flex align-items-center gap-2">
                  <span
                    class="condition-name"
                    :style="{ color: item.isDefault === 1 ? '#02b96b' : '#606266' }"
                  >
                    {{ item.conditionName }}
                  </span>
                  <Tag color="gray" v-if="item.isDefault === 1">默认</Tag>
                </div>

                <div class="d-flex align-items-center gap-1 ms-auto">
                  <span
                    :class="item.isDefault === 1 ? 'color-transparent' : 'view-set-default-tag'"
                    @click.stop.prevent="setDefaultView(item.conditionCode!)"
                  >
                    设为默认
                  </span>
                  <i
                  ...
                  />
                  <i
                    class="iconfont icon-shanchu text-danger"
                    @click.stop.prevent="handleDeleteView(item.conditionCode!)"
                  />
                </div>
              </div>
            </el-dropdown-item>
            <el-dropdown-item class="text-center" v-if="!viewList?.length">
              <span>暂无数据</span>
            </el-dropdown-item>
          </el-dropdown-menu>
        </template>
    ...
    <template #buttons>
      ...
      />
      <el-button type="primary" @click="searchJijiaOrder">查看积加自发货订单预占</el-button>
    </template>
    ...
    <template #default="{ tableHeight }">
      ...
      >
        ...
        <vxe-column field="orderInfo" min-width="743" class-name="order-info-column">
          <template #header>
          ...
          <template #default="{ row }">
            <div class="table-item-header first-header">
              <div class="item">
                ...
                <div>{{ row.shopAccount || '-' }}:{{ row.site || '-' }}</div>
              </div>
              <div class="item">
                <span class="item-label">平台单号：</span>
                <span class="item-value" @click="gotoDetail(row)">{{ row.refNo || '-' }}</span>
                ...
                <div class="left-top">
                  ...
                  <div class="product-list">
                    ...
                    >
                      ...
                      <div class="num">
                        <ItemLabel label="数量" :value="product?.numInt" />
                      </div>
                      <div class="price">
                        ...
                        <ItemLabel label="商品买家支付运费" :value="product?.salesFreightAmount" />
          ...
          <template #default="{ row }">
            ...
            <ChildColumn title="发货信息" :custom-field-list="customFieldList" :row="row">
              <template #buyerCountryDesc>
                <div class="d-flex">
                  <ItemLabel label="国家" :value="row?.buyerCountryDesc" />
                ...
                :actions="[
                  {
                    ...
                    hide:
                      ...
                      !(
                        [13, 11, 12].includes(row.source) &&
                        [ErrorStatus.缺货].includes(row.orderStatus)
                      ),
                    row: row,
                  },
                  {
                    ...
                    text: true,
                    hide:
                      row.orderStatus !== ErrorStatus.禁止处理 || [13, 12, 11].includes(row.source),
                    row: row,
                  },
                  {
                    label: '转自配送',
                    ...
                    text: true,
                    hide:
                      ![ErrorStatus.转FBA配送异常, ErrorStatus.转FBA创建失败].includes(
                        row.orderStatus
                      ) || row.source == 12,
                    row: row,
                  },
                  {
                    ...
                    hide:
                      ![
                        ...
                        ErrorStatus.预占失败,
                        ErrorStatus.提交出库失败,
                      ].includes(+row.orderStatus) || [13, 12, 11].includes(row.source),
                    isMore: true,
                    row: row,
                  ...
                  },
                  {
                    ...
                    text: true,
                    hide:
                      ![OrderStatus.已发货].includes(+row.orderStatus) ||
                      [13, 12, 11].includes(row.source),
                    isMore: false,
                    row: row,
                    // permissionCode: 'B2BOrderEdit',
                  },
                  ...
                  },
                  {
                    ...
                    text: true,
                    hide:
                      [OrderStatus.已发货, OrderStatus.废弃].includes(+row.orderStatus) ||
                      ([13, 11].includes(row.source) &&
                        ![OrderStatus.待审核, OrderStatus.待系统内审核].includes(
                          row.orderStatus
                        )) ||
                      (row.source == 12 &&
                        [OrderStatus.废弃, OrderStatus.已发货, ErrorStatus.取消预占失败].includes(
                          row.orderStatus
                        )),
                  ...
                  // (row.source == 12 && ![OrderStatus.待审核, OrderStatus.待系统内审核, ErrorStatus.缺货, ErrorStatus.预占失败].includes(row.orderStatus))
                  {
                    label: '拆单',
                    key: 'subOrder',
                    type: 'primary',
                  ...
                  },
                  {
                    ...
                    hide:
                      ![
                        ...
                        OrderStatus.待系统内审核,
                        OrderStatus.待审核,
                      ].includes(row.orderStatus) || row.source == 12,
                  ...
                  },
                  {
                    ...
                    row: row,
                    hide:
                      ![MarkShipedStatus.标发失败].includes(row.markShippedStatus) ||
                      [13, 12, 11].includes(row.source),
                    // permissionCode: 'B2BOrderDetail',
                  },
                  {
                    ...
                    row: row,
                    hide:
                      ![MarkShipedStatus.标发成功].includes(row.markShippedStatus) ||
                      [13, 12, 11].includes(row.source),
                  },
                  {
                    label: '重新物流下单',
                  ...
                  },
                  {
                    ...
                    hide:
                      ![
                        ...
                        ErrorStatus.三方仓SKU未配对,
                        ErrorStatus.预占失败,
                      ].includes(row.orderStatus) || [13, 11].includes(row.source),
                    isMore: true,
                    row: row,
                  },
                  {
                    ...
                    type: 'primary',
                    text: true,
                    hide: ![ErrorStatus.提交出库失败].includes(row.orderStatus),
                    isMore: false,
                    row: row,
                  },
                  {
                  ...
                  },
                  {
                    ...
                    hide:
                      ...
                      ([11, 12].includes(row.source) && row?.orderStatus == OrderStatus.待发货),
  ...
  <FbaOrderPushDialog
    ...
    "
  />
  <BatchFbaOrderPushDialog
    ref="batchFbaOrderRef"
    @success="
      () => {
        refreshList();
        clearCheckBoxSelection();
  ...
  <OrderAddDialog ref="OrderAddDialogRef" @success="handleClose" />
  ...
  <StoreView ref="storeViewRef" @saveView="handleAddView" />
</template>
...
<script lang="ts" setup name="shopifyOrder">
  ...
  import useList from './components/useShopifyOrderList';
  ...
  import customFields from '@/views/supplyChainManage/B2BOrder/shopifyOrder/customFields';
  ...
  import BatchFbaOrderPushDialog from '@/views/supplyChainManage/B2BOrder/shopifyOrder/components/batchFbaOrderPushDialog.vue';
  ...
  import AddressDetail from '@/views/supplyChainManage/B2BOrder/shopifyOrder/components/addressDetail.vue';
  ...
  import { OrderStatus, ErrorStatus, MarkShipedStatus } from './enum';
  import { useStoreView } from '@/hooks/event/useStoreView';

  const storeViewRef = ref<InstanceType<typeof StoreView> | null>(null);
  const MAX_VIEW_COUNT = 10;
  const currentView = ref<any>();
  const handleSaveView = () => {
    if (viewList.value.length >= MAX_VIEW_COUNT) {
      ElMessage.warning('视图列表最多存10条，请先删除视图后重试');
      return;
    }
    storeViewRef.value?.open({ viewList: viewList.value });
  };
  const handleAddView = async ($event: any) => {
    await addView($event, search);
    currentView.value = { conditionName: $event.name, searchCondition: { ...search } };
  };
  const handleCommand = (command: string) => {
    const viewData = viewList.value.find((item) => item.conditionCode === command);
    if (!viewData) {
      return;
    }
    currentView.value = viewData;
    const { searchCondition } = viewData;
    Object.keys(search).forEach((key) => {
      search[key] = searchCondition![key];
    });
  };
  const {
    loading: viewLoading,
  ...
  const suspendStatusSelectRef = ref(null);
  ...
  const {
    ...
    interceptSearchData: (searchData) => {
      ...
      return {
        ...
        sortParam: sort,
        itemSingleSearchKey:
          sendParam.itemSingleSearchKey?.length >= 5 ? sendParam.itemSingleSearchKey : undefined,
      };
    },
    serviceManual: true,
  });
  initView(() => {
    const defaultView = viewList.value.find((item) => item.isDefault === 1);
    if (defaultView) {
      const { searchCondition, conditionName } = defaultView;
  ...
  /** 当前选中视图有值时，若查询条件与视图保存条件不一致则清空 currentView */
  const snapshotSearchForCompare = (o: Record<string, any>) => JSON.parse(JSON.stringify(o));
  const clearCurrentViewIfSearchDrifted = () => {
    const view = currentView.value;
    if (!view?.searchCondition) return;
    const saved = view.searchCondition;
    if (saved === null || typeof saved !== 'object' || Array.isArray(saved)) return;
    try {
      if (!isEqual(snapshotSearchForCompare(search), snapshotSearchForCompare(saved))) {
        currentView.value = null;
      }
    } catch {
      if (!isEqual(search, saved)) {
        currentView.value = null;
      }
    }
  };
  watch(search, clearCurrentViewIfSearchDrifted, { deep: true });

  const handleDeleteView = async (conditionCode: string) => {
    const isConfirmed = await swal.confirm('确定删除该视图吗？');
    if (isConfirmed) {
      await deleteView(conditionCode);
      currentView.value = null;
  ...
  const selectedList = computed(() => {
    return [
      ...
      },
      {
        label: '禁止处理',
        key: 'suspendStatus',
        value: suspendStatusSelectRef.value?.selectedLabel,
      },
      {
        label: '异常信息',
        key: 'errorRemark',
  ...
  const handleActions = async (operation) => {
    ...
    switch (key) {
      ...
      case 'replace': {
        const path = router.resolve({
          name: 'shopifyOrderEdit',
          query: { code: row.erpCode, replace: true },
        });
        router.push(path.href);
        break;
      }
  ...
  const handleBatchMarkShipped = async () => {
    ...
    // 领用订单、内购订单、红人订单
    const receiveList = getCheckedList()?.filter((item) => [13, 12, 11].includes(item.source));
    if (receiveList.length > 0) {
      ElMessage.warning('领用订单、内购订单、红人订单不支持【批量标记发货】操作');
      return;
    }
  ...
  const handleBatchUpdateMarkShipped = async () => {
    ...
    const receiveList = getCheckedList()?.filter((item) => [13, 12, 11].includes(item.source));
    if (receiveList.length > 0) {
      ElMessage.warning('领用订单、内购订单、红人订单不支持【批量更新标发】操作');
      return;
    }
  ...
  const batchReplacement = () => {
    // 领用订单、红人订单
    const receiveList = getCheckedList()?.filter((item) => [13, 11].includes(item.source));
    if (receiveList.length > 0) {
      ElMessage.warning('领用订单、红人订单不支持【批量换货】操作');
      return;
    }
  ...
  const batchSubOrder = async () => {
    ...
    const receiveList = getCheckedList()?.filter((item) => [13, 12, 11].includes(item.source));
    if (receiveList.length > 0) {
      ElMessage.warning('领用订单、红人订单、内购订单不支持【批量拆单】操作');
      return;
    }
    // const inAppList = getCheckedList()?.filter(item => item.source == 12 && ![
    //       OrderStatus.待系统内审核,
    //       OrderStatus.待审核,
    //       ErrorStatus.缺货,
    //       ErrorStatus.预占失败,
    ...
    //   ElMessage.warning('内购订单仅缺货、预占失败、待审核支持【批量拆单】操作');
    //   return;
    // }
    const illegalList = getCheckedList()?.filter(
      (item) =>
    ...
    return [
      ...
      },
      {
        ...
        fn: () => {
          ...
          // 领用订单、红人订单、内购订单
          const receiveList = getCheckedList()?.filter((item) =>
            [13, 12, 11].includes(item.source)
          );
          if (receiveList.length > 0) {
            ElMessage.warning('领用订单、红人订单、内购订单不支持【更新MSKU配对信息】操作');
            return;
          }
      ...
      },
      {
        ...
        fn: () => {
          ...
          if (receiveList.length > 0) {
            ElMessage.warning('领用订单、内购订单、红人订单不支持【重新执行订单规则】操作');
            return;
          }
      ...
      },
      {
        ...
        fn: () => {
          ...
          // 领用订单、红人订单
          const receiveList = getCheckedList()?.filter(
            (item) =>
              [13, 11].includes(item.source) &&
              ![OrderStatus.待审核, OrderStatus.待系统内审核].includes(item.orderStatus)
          );
          if (receiveList.length > 0) {
            ElMessage.warning('领用订单、红人订单仅待审核支持【作废】操作');
            return;
          }
          // 内购订单
          const inAppList = getCheckedList()?.filter(
            (item) =>
              item.source === 12 &&
              [OrderStatus.废弃, OrderStatus.已发货, ErrorStatus.取消预占失败].includes(
                item.orderStatus
          ...
          if (inAppList.length > 0) {
            ElMessage.warning('内购订单废弃、取消预占失败、已发货不支持【作废】操作');
            return;
          }
          abandonOrder();
        },
  ...
  const handleBatchForbid = () => {
    // 领用订单、红人订单、内购订单
    const receiveList = getCheckedList()?.filter((item) => [13, 11, 12].includes(item.source));
    if (receiveList.length > 0) {
      ElMessage.warning('领用订单、红人订单、内购订单不支持【禁止处理】操作');
      return;
    }
    const illegalList = getCheckedList()?.filter(
      (item) =>
  ...
  const handleBatchAddGift = () => {
    ...
    const receiveList = getCheckedList()?.filter((item) => [13, 11, 12].includes(item.source));
    if (receiveList.length > 0) {
      ElMessage.warning('领用订单、红人订单、内购订单不支持【添加赠品】操作');
      return;
    }
  ...
  const submitOutbound = async (erpCode = null) => {
    ...
    if (!erpCode) {
      ...
      if (illegalList.length > 0) {
        ElMessage.warning('只有待审核、缺货、预占失败、提交出库失败的订单支持【提交发货】操作');
        return;
      }
    }
      ...
      beforeClose: async (action, instance, done) => {
        ...
        if (action === 'confirm') {
          ...
          try {
            ...
            // }
            const go = await swal.taskConfirm('异步处理中，请前往任务列表查看发货审核结果');
            if (go) {
              router.push({ path: '/taskDetail' });
            }
            ...
            const go = await swal.taskConfirm('操作失败，请前往任务列表查看发货审核结果');
            if (go) {
              router.push({ path: '/taskDetail' });
            }
  ...
  const batchShipping = (erpCodeList) => {
    ...
    const receiveList = getCheckedList()?.filter((item) => [13, 11, 12].includes(item.source));
    if (receiveList.length > 0) {
      ElMessage.warning('领用订单、红人订单、内购订单不支持【设置仓库物流】操作');
      return;
    }
    const illegalList = getCheckedList()?.filter(
      (item) =>
  ...
  const transferSelf = (erpCodeList, skuList) => {
    ...
    // 内购订单
    const receiveList = getCheckedList()?.filter((item) => item.source == 12);
    if (receiveList.length > 0) {
      ElMessage.warning('内购订单不支持【转自配送】操作');
      return;
    }
    const illegalList = getCheckedList()?.filter(
      (item) =>
  ...
  const transferFba = (erpCodeList, skuList) => {
    if (!erpCodeList?.length && !batchIntercept()) {
      return;
    }
    const illegalList = getCheckedList()?.filter(
    ...
    if (illegalList.length > 0) {
      ElMessage.warning('只有待审核、待系统内审核、异常的订单支持【批量转FBA】操作');
      return;
    }
    // console.log(erpCodeList, skuList);
    batchFbaOrderRef.value?.open(erpCodeList, skuList, rowList.value);
  };
  // 相同的 供应链sku , 数量合并
  function mergeBySupplySkuWithMap(skuList) {
    const map = new Map();
    skuList.forEach((item) => {
      const existingItem = map.get(item.supplySku);
      if (existingItem) {
        // 累加数量
        existingItem.numInt += item.numInt;
        existingItem.num = `x ${existingItem.numInt}`;
      } else {
        // 创建新项目并存入 Map
        map.set(item.supplySku, { ...item });
      }
    });
    // 将 Map 的值转换为数组
    return Array.from(map.values());
  }
  const batchTransferFba = () => {
    const skuList = rowList.value?.flatMap((row) => row.itemBeanList);
    // console.log('skuList', skuList);
    let skuMergeList = mergeBySupplySkuWithMap(skuList);
    // console.log('skuMergeList', skuMergeList);
    transferFba(
      codeList.value,
      // rowList.value?.flatMap((row) => row.itemBeanList?.map((item) => item.supplySku))
      skuMergeList
  ...
  const exportFunc = async (type?) => {
    try {
      ...
      const res = await omsApi.omsOrderOrderDownload({
        ...
        exportDimension: type === 'order' ? 'header' : '',
      });
  ...
  // 查看积加
  const searchJijiaOrder = (item) => {
    router.push('/jijiaSelfShippedOrder');
  };
...
<style scoped lang="scss">
  ...
  :global(.shopify-save-view-dropdown .view-save-dropdown-name-cell > :first-child) {
    min-width: 0;
    flex: 1 1 0%;
  }

  :global(.shopify-save-view-dropdown .view-save-dropdown-row .view-set-default-tag) {
    opacity: 0;
    transition: opacity 0.15s ease;
    pointer-events: none;
  }
  :global(.shopify-save-view-dropdown .view-save-dropdown-row:hover .view-set-default-tag) {
    opacity: 1;
    pointer-events: auto;
  }
