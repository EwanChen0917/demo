<template>
  <div class="bg-wrap" ref="bgWrapRef">
    <!-- <div class="bdr"> -->
    <div class="aewrap" ref="aewrapRef">
      ...
      <el-form :model="form" ref="formRef" :rules="rules" label-width="140px" label-position="top">
        ...
        <div class="tabwrap">
          ...
          <div class="wrap1" id="product" ref="productWrapperRef">
            <div :class="isSticky ? 'add-title3 product-sticky' : 'add-title3'">
            ...
            <div :class="isSticky ? 'tablewrap tablewrap-sticky' : 'tablewrap'">
              ...
              >
                    ...
                    <div class="product-item">
                      <div class="left">
                        ...
                        <div>
                          ...
                          <div class="product-sku" v-if="!isSpsedi">
                            <span class="item-label">产品SKU：</span>
                            {{ row?.skuCode }}
                          </div>
                          <div class="product-sku" v-else>
                            <span class="item-label">产品SKU：</span>
                            {{ row?.skuCode }}
                            <el-button
                              type="primary"
                              @click="handleOpenProductDialog(row, $index)"
                              size="small"
                            >
                              更换产品
                            </el-button>
                            <!-- <el-select
                              ...
                              :loading="skuLoading"
                              class="w-100"
                              @change="handleSkuChange($event, row, $index)"
                ...
                <el-table-column v-else prop="num" label="数量 (件)" width="110">
                  <template #header>
                    <span>数量 (件)</span>
                    <BatchSetPopover
                      :form-data="form"
                  ...
                  <template #default="{ row, $index }">
                    ...
                    >
                      ...
                      <div class="text-warning fontcolor d-flex flex-align timeicon">
                        ...
                        预估时效：
                        <template v-if="row.shippingCode?.split(',')?.length === 1">
                          {{
                            row?.shippingList?.find((v) => v.value === row.shippingCode)?.params
                              ?.timeliness
                          }}
                        </template>
                        <template v-if="row.shippingCode?.split(',')?.length === 2">
                          {{
                            row?.spList?.find((v) => v.spCode === row.shippingCode.split(',')[0]) &&
                            row?.spList
                              ?.find((v) => v.spCode === row.shippingCode.split(',')[0])
                              .shippingList?.find((v) => v.value === row.shippingCode.split(',')[1])
                              ?.params?.timeliness
                          }}
                        </template>
                      </div>
      ...
      <div class="d-flex justify-content-center footer fixed">
        ...
        <el-button type="primary" :loading="saveLoading" @click="save(true)">
          {{
            isEdit
              ? [516, 517].includes(form?.baseInfo?.orderStatus)
                ? '确定'
                : '确定并转待审核'
              : isAdd
              ? '提交订单'
      ...
      <ProductDialog2
        ...
        :currencyCode="form.baseInfo.currencyCode"
        :searchErpQuotation="true"
        v-if="productDialogVisible2"
        :visible="productDialogVisible2"
        @close="handleSkuChange"
        :selectOne="true"
        confirm-text="确认"
        title="更换产品"
        type="supplierSku"
...
<script setup lang="ts" name="B2BOrderNewAdd">
  import Num from '@/components/NumberInput/index.vue';
  import { erpApi, financeSystemApi, productApi, omsApi } from '@/api';
  ...
  import BatchSetPopover from '@/views/supplyChainManage/B2BOrder/orderManageNew/components/BatchSetPopover.vue';
  import ProductDialog2 from '@/views/supplyChainManage/B2BOrder/orderManageNew/components/productDialog2.vue';
  // import * as swal from '@/utils/swal';
  import { cloneDeep } from 'lodash-es';
  import { ElMessage, ElMessageBox, FormInstance, FormRules } from 'element-plus';
  import { priceRuleFixed4 } from '@/utils/rulesRegExp';
  import { useTabs } from '@/hooks/tabs';
  import CancelDialog from '@/views/supplyChainManage/B2BOrder/orderManageNew/components/CancelDialog.vue';
  ...
  onMounted(() => {
    ...
    initScroll();
    scrollContainerRef.value = document.querySelector('.layout-right-main');
    init();
    // resetFormData();
  });
  onUnmounted(() => {
  ...
  const cancelSaveAndReset = () => {
    closeTabAndRefresh('B2BOrderNew');
    goBackOrder();
    resetFormData();
  };
  ...
  const isSpsedi = ref(false);
  // const itemInfoListBackup: any = ref();
  const initData = async () => {
    ...
    if (res?.baseInfo?.source === 6) {
      isSpsedi.value = true;
    } else {
      isSpsedi.value = false;
    }
    // itemInfoListBackup.value = JSON.parse(JSON.stringify(res?.itemInfoList));
    // console.log('itemInfoListBackup', itemInfoListBackup.value);
    salesmanCode.value = res.customerInfoBean?.maintainCode;
    // console.log('salesmanCode :', salesmanCode.value);
    // 订单备注
    ...
    // console.log(form.value?.baseInfo?.source, form.value?.baseInfo?.sourceDesc);
    // source 8  表示 来源 team sync新增
    if (form.value?.baseInfo?.source === 8) {
      orderSaleTypeMap.value = [];
      // orderSaleTypeMap.value.push({ value: 'To C', label: 'To C' });
      orderSaleTypeMap.value.push({
        value: res.baseInfo?.orderSaleType,
        label: res.baseInfo?.orderSaleType,
      });
    }
  ...
  const initCountrySelection = async (val) => {
    try {
      // loading.value = true;
      const res = await omsApi.omsOrderReferenceParams({ type: 1, countryCode: val });
      if (isAdd.value) {
  ...
  const productDialogVisible2 = ref(false);
  // 选择商品
  const handleProductDialogClose = async (selected: boolean, list: any[]) => {
    ...
    productDialogRef.value?.closeLoading();
  };
  // let isToast = false;
  // 判断是否高亮
  ...
  const checkCartonInfo = async (skuCodeList, clearType) => {
    const cartonSize = await omsApi.omsCartonSizeQueryList({
  ...
  const handleChange = (code) => {
    ...
    // 新传业务人员编码 salesmanCode  = 客户列表返回的maintainCode带过来
    form.value.receiveInfo.salesmanCode = customerInfo?.maintainCode;
    salesmanCode.value = customerInfo?.maintainCode;
  ...
  const handleWarehouseCodeChange = async (val: string, currentRow: any, currentWarehouseList) => {
    ...
    // currentRow.shippingCode = '';
    currentRow.supplySku = '';
    const res: any = await omsApi.omsCommonStockQuerySupplySkuList({
  ...
  const handleAddProduct = () => {
    if (isSpsedi.value) {
      ElMessage.warning('EDI新增订单，请在订单行修改SKU');
      return;
    }
    productDialogVisible.value = true;
  };
  const skuOptions: any = ref([]);
  const skuLoading = ref(false);
  const remoteSKU = async (query: string, index?, sku?) => {
    // 代存修改前的 sku 数据
    console.log('remoteSKU', query, index, sku);
    // if (query) {
    ...
    let param = {
      ...
      searchErpQuotation: true,
      orderType: form.value.receiveInfo.customerCode ? 1 : 0,
      share: 2,
      salesmanCode: salesmanCode.value,
      pageNum: 1,
      pageSize: 10,
    ...
    const res = await omsApi.omsCommonStockQuerySkuStock(param);
    skuLoading.value = false;
    skuOptions.value = res?.skuStockList;
  ...
  // remoteSKU(' ');
  let skuIndex = -1;
  const handleOpenProductDialog = (row?, index?) => {
    productDialogVisible2.value = true;
    skuIndex = index;
    // console.log('skuIndex', skuIndex);
  };
  const handleSkuChange = async (val: any, selectSku?, index?) => {
    // console.log('Change', val, selectSku);
    // console.log('skuIndex', skuIndex);
    // return;
    // const selectSku = skuOptions.value.find((item) => item.skuCode === val);
    // console.log(selectSku, 'selectSku', skuIndex);
    if (skuIndex < 0) {
      // console.log('下标不存在', skuIndex);
      ElMessage.warning('请选择需要更改的商品');
      productDialogVisible2.value = false;
      return;
    }
    if (!selectSku) {
      // console.log('selectSku 没有更换');
      productDialogVisible2.value = false;
      return;
    }
    const row: any = form.value.itemInfoList[skuIndex];
    row.productName = selectSku.productTitle;
    row.productImageUrl = selectSku.productImageUrl;
    row.suggestPrice = selectSku.suggestPrice;
    row.skuCode = selectSku.skuCode;

    row.inventory = 0;
    row.warehouseList = [];
    row.supplySkuItemList = [];
    row.shippingList = [];
    row.deliveryList = [];
    row.deliveryCode = '';
    row.num = 0;
    ...
    row.supplySku = '';
    row.warehouseCode = '';
    row.unitCode = unitCode.value;
    handleProductDialogClose2(true, [row], index);
  };
  // 直接修改 商品sku
  const handleProductDialogClose2 = async (selected: boolean, list: any[], index?) => {
    // productDialogVisible2.value = false;
    // console.log(list);
    // return;
    productDialogVisible2.value = false;
  ...
  const handleAutoWarehouse = async () => {
    const ruleHit = await omsApi.omsOrderWarehouseRuleHit({
  ...
  const save = async (isSubmit) => {
    ...
    if (trueArr.length === valid.length) {
      if (!form.value.itemInfoList.length) {
        ...
        // console.log('itemInfoList', form.value.itemInfoList);
        const params = cloneDeep(form.value);
        params.itemInfoList.forEach((item) => {
        ...
        try {
          if (isAudit.value) {
            await omsApi.omsOrderAuthAndUpdate(params).finally(() => {
            ...
            if (isSubmit) {
              await omsApi.omsOrderSave(params as any).finally(() => {
                saveLoading.value = false;
              });
            } else {
              // console.log('新增 存草稿');
              await omsApi.omsOrderSaveDraft(params as any).finally(() => {
                saveLoading.value = false;
              });
            }
          }
          // await (isAudit.value
          //   ? omsApi.omsOrderAuthAndUpdate(form.value)
          //   : isEdit.value
          //   ? omsApi.omsOrderUpdate({
          //       ...form.value,
          //       submit: isSubmit,
          //     })
          //   : omsApi.omsOrderSave(form.value)
          // ).finally(() => {
          //   saveLoading.value = false;
          // });
          if (isAdd.value) {
          ...
          // console.log(error);
          if (error.message.indexOf('推送第三方系统异常') > -1) {
            closeTabAndRefresh('B2BOrderNew');
            goBackOrder();
          }
        }
  ...
  const handleCurrencyChange = async () => {
    const res = await omsApi.omsQuotationPriceGetSuggestPrice2({
      currencyCode: form.value.baseInfo.currencyCode,
      customerCode: form.value.receiveInfo.customerCode,
  ...
  const warehouseRes = ref();
  const handleWarehouseCodeColumnChange = async (
    val: string,
    currentRow: any,
    ...
    // currentRow.shippingCode = '';
    // currentRow.supplySku = '';
    if (!warehouseRes.value) {
      warehouseRes.value = await omsApi.omsCommonStockQuerySupplySkuList({
        deliverType: currentRow.deliveryCode,
        skuCode: currentRow.skuCode,
        warehouseCode: val,
        salesmanCode: salesmanCode.value,
    ...
    if (warehouseRes.value) {
      currentRow.supplySkuItemList = warehouseRes.value.supplySkuItemList;
      currentRow.supplySku = currentRow.supplySkuItemList[0]
        ? currentRow.supplySkuItemList[0].supplySku
        : '';
      currentRow.inventory = currentRow.supplySkuItemList[0]
  ...
  const handleColumnChange = (key, value = '') => {
    (form.value.itemInfoList as Array<any>) = form.value.itemInfoList.map((item: any) => {
      if (key == 'warehouseCode') {
        item.deliveryCode = form.value.itemInfoList[0].deliveryCode;
        item.deliveryList = form.value.itemInfoList[0].deliveryList;
        item.warehouseList = form.value.itemInfoList[0].warehouseList;
      }
      if (key == 'shippingCode') {
        item.deliveryCode = form.value.itemInfoList[0].deliveryCode;
        item.warehouseCode = form.value.itemInfoList[0].warehouseCode;
        item.deliveryList = form.value.itemInfoList[0].deliveryList;
        item.warehouseList = form.value.itemInfoList[0].warehouseList;
        item.spList = form.value.itemInfoList[0].spList;
      }
    ...
    if (key == 'deliveryCode') {
      form.value.itemInfoList.forEach((item: any) => {
        handleDeliveryCodeChange(value, item, item.deliveryList);
      });
    }
    if (key == 'warehouseCode' || key == 'shippingCode') {
      warehouseRes.value = null;
      form.value.itemInfoList.forEach((item: any) => {
        const code = key == 'warehouseCode' ? value : item.warehouseCode;
        handleWarehouseCodeColumnChange(code, item, item.warehouseList, key);
      });
      resetStatistics();
    }
    if (key == 'cartonQty') {
...
<style scoped lang="scss">
  ...
  // 产品信息表格 - 表头未吸顶状态（默认状态）
  :deep(.product-table-header-normal) {
    // 默认状态可以不添加额外样式，或根据需要添加
  }
  ...
  :deep(.product-table-header-sticky) {
    .el-table__header-wrapper {
      position: fixed !important;
      top: var(--table-header-top, 254px) !important;
      width: var(--table-header-width, 100%) !important;
      z-index: 999;
      background: white;
