...
<script lang="ts" setup>
  ...
  const modifyService = ref();

  const open = (codeList, serivce) => {
    orderList.value = codeList;
    modifyService.value = serivce;
    queryList(codeList);
    visible.value = true;
    form.value = {
      deliveryCode: '',
  ...
  const queryList = async (orderCodeList) => {
    ...
    try {
      ...
      const res = await erpApi.luteosErpOrderQueryWarehouseLogistics({
  ...
  const handleFormSubmit = async () => {
    ...
    if (valid) {
      try {
        ...
        const service = modifyService.value || erpApi.luteosErpOrderModifyWarehouseLogistics;
        await service({
          ...form.value,
          erpCodeList: orderList.value,
        });
