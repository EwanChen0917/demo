...
<script setup lang="ts">
  ...
  if (checkPermission('orderAddUpload')) {
    sceneList.value.push({
      desc: '新增订单',
      service: erpApi.luteosErpOrderAddUpload,
  ...
  if (checkPermission('updateAddressUpload')) {
    sceneList.value.push({
      desc: `更改地址`,
      service: erpApi.luteosErpOrderUpdateAddressUpload,
  ...
  if (checkPermission('orderAttachUpload')) {
    sceneList.value.push({
      desc: `上传订单附件`,
      service: erpApi.luteosErpOrderOrderAttachUpload,
      fileList: [],
    });
  }
...
res.failedCount > 0 ? '，请到上传日志查看' : ''
  ...
  const handleSearch = async () => {
    await formRef.value?.validate(async (valid) => {
      if (!valid) return;
      try {
        const res = await erpApi.luteosErpEdiQueryList({
          pageNum: 1,
          pageSize: 10,
