<template>
  ...
  >
    <template #search>
      <lt-batch-search
        ref="batchSearchRef"
        :prepend-content-list="searchType"
        @search="handleBlur"
    ...
    <template #filters>
      ...
      <lt-batch-search
        ...
        width="207px"
        @search="handleBlur2"
        @batch-search="handleBatchSearch2"
        @change="handleChange"
      />
    ...
    <template #buttons>
      <el-button type="success" @click="router.push('/B2BOrder/add')" v-permission="'B2BOrderAdd'">
        新增订单
      </el-button>
      <!-- <ErpStockPopover
        name="查询ERP库存可用量"
      ...
      >
        <template #reference>
          <el-button @click="stockPopoverRef.open()">ERP库存可用量</el-button>
        </template>
      </ErpStockPopover>
    ...
    <template #default="{ tableHeight }">
      ...
      >
        ...
        <vxe-column title="操作" width="52px" fixed="right">
          <template #default="{ row }">
            <div class="table-item-header"></div>
              ...
              <KeenActions
                ...
                :actions="[
                  ...
                  },
                  {
                    ...
                    type: 'primary',
                    text: true,
                    hide: ![102, 503, 506].includes(+row.orderStatus),
                    // hide: !(
...
<script lang="ts" setup name="B2BOrder">
  import { erpApi, memberApi, ErpApi, platformApi } from '@/api/index';
  import { useRouter } from 'vue-router';
  import * as swal from '@/utils/swal';
  import { ElMessage, ElSelect, LtBatchSearch } from 'element-plus';
  ...
  import customFields from '@/views/supplyChainManage/B2BOrder/orderManage/customFields';
  ...
  import ExportOriginDialog from '@/views/supplyChainManage/B2BOrder/orderManage/components/ExportOriginDialog.vue';
  import ExportOriginDialog2 from '@/views/supplyChainManage/B2BOrder/orderManage/components/ExportOriginDialog2.vue';
  import SubOrderDialog from '@/views/supplyChainManage/B2BOrder/orderManage/components/SubOrderDialog.vue';
  import InventoryPreCheckDialog from '@/views/supplyChainManage/B2BOrder/orderManage/components/InventoryPreCheckDialog.vue';
  import BatchReplacementDialog from '@/views/supplyChainManage/B2BOrder/orderManage/components/BatchReplacementDialog.vue';
  import BatchToDeliveryDialog from '@/views/supplyChainManage/B2BOrder/orderManage/components//BatchToDelivery.vue';
  import QueryStockDialog from '@/views/supplyChainManage/B2BOrder/orderManage/components/QueryStock.vue';
  ...
  // 自定义字段由 KeenList 内部托管（auto-custom-fields），页面通过 v-model 拿到结果用于列显隐
  const conditionCode = ref<string | undefined>();
  const exportConditionCode = ref<string | undefined>();
  const customFieldList = ref<any[]>(customFields);
  const exportCustomFieldList = ref<any[]>(customExportFields);
  ...
  const searchType = Object.keys(searchTypeMap).map((item) => {
    return { label: searchTypeMap[item], value: item };
  });
  ...
  const clearAll = () => {
    ...
    batchSearchRef.value?.clearTextArea();
    batchSearchRef2.value?.clearTextArea();
  };
  ...
  const handleBlur = (value, selectedValue) => {
    search.orderCode = value;
    search.searchType = selectedValue;
    // batchSearchRef.value?.clearTextArea();
  };
  ...
  const handleBatchSearch = (value, selectedValue) => {
  ...
  const handleBatchSearch2 = (val) => {
    ...
    const arr = val.split(',').filter((item) => item.length > 0);
    // console.log(arr);
    if (arr.length === 0) {
  ...
  const handleSaveCustomerRemark = async (row) => {
    try {
      ...
      const res = await erpApi.luteosErpOrderUpdateCustomRemark({
        erpCode: row?.erpCode,
  ...
  const batchReplacement = () => {
    const illegalList = tableRef.value
      ?.getCheckboxRecords()
      ?.filter((item) => search.orderBigStatus !== '100' && item?.orderStatus !== 506);
    if (illegalList.length > 0) {
      ElMessage.warning('存在非待确认/缺货(OS)订单，请重新选择');
      return;
    }
  ...
  const handleBatchSetStock = () => {
    ...
    if (illegalList.length > 0) {
      ElMessage.warning('存在非待确认/缺货(OS)/预占失败/提交出库失败订单，请重新选择');
      return;
    }
  ...
  const batchTransferToPendingShipment = async () => {
    const illegalList = tableRef.value
      ?.getCheckboxRecords()
      ?.filter((item) => item?.orderStatus !== 515);
    if (illegalList.length > 0) {
    ...
    if (!isConfirmed) return;
    const res = await erpApi.luteosErpOrderBatchToWaitSend({
      orderCodeList: codeList.value,
    });
    if (res?.failList.length > 0) {
      const content = res.failList
  ...
  const rePush = async (codeList) => {
    ...
    const res = await erpApi.luteosErpOrderRePush(codeList);
    if (res?.failList.length > 0) {
      const content = res.failList
        .map((item) => {
          return `订单号：${item.orderCode}，失败原因：${item.message}`;
  ...
  const getStockStatus = async (orderList) => {
    ...
    try {
      const res = await erpApi.luteosErpOrderQueryStockList(orderCodes);
      if (res?.length) {
  ...
  const exportOriginDialogRef2 = ref(null);
...
<style scoped lang="scss">
  ...
  :global(.orderSaleSelect .el-tag) {
    max-width: 80px !important;
  }
  :global(.countrySelect .el-tag) {
