          ...
          <KeenCard title="设定动作">
            ...
            <div class="padb20">
              <div class="d-flex align-items-center">
                <div class="noshrink tips">门牌号解析:</div>
                <div class="padl10" v-if="formModel.houseNumberCheck">
                  {{ formModel.actionParams.receiveHouseNumber.desc || '-' }}
                </div>
                <div v-else class="houseNumber bg3"><div class="checkItem">-</div></div>
              </div>
            </div>
            <div class="padb20 d-flex">
              <div class="noshrink tips">电话号码简化:</div>
              <div class="houseNumber bg3" v-if="formModel.telephoneCheck">
                <div class="checkItem" v-if="formModel.telephoneAnalysis?.includes(1)">
                  {{
                    formModel.actionParams.receivePhone.extNumber
                      ? `分机号处理【${
                          formModel.actionParams.receivePhone.extNumber == '1'
                            ? '将“ext.”（包括前后的空格）替换为“-”'
                            : formModel.actionParams.receivePhone.extNumber == '2'
                            ? '去掉分机号（仅支持ext.格式）'
                            : ''
                        }】`
                      : '-'
                ...
                <div class="checkItem" v-if="formModel.telephoneAnalysis?.includes(2)">
                  {{
                    `过滤特殊字符【${formModel.actionParams.receivePhone?.filterSpecialStrList}】`
                  }}
                </div>
                <div
                  class="checkItem"
                  v-if="formModel.actionParams.receivePhone.filterTopZeroNumber"
                >
                  过滤头部为0的数字
                </div>
                <div class="checkItem" v-if="formModel.telephoneAnalysis?.includes(4)">
                  {{
                    `从第【${formModel.actionParams.receivePhone?.subBeginIndex}】个字符开始截取`
                  }}
                </div>
                <div class="checkItem" v-if="formModel.telephoneAnalysis?.includes(5)">
                  {{ `电话号码替换为【${formModel.actionParams.receivePhone?.replaceAllStr}】` }}
                </div>
                <div class="checkItem" v-if="formModel.telephoneAnalysis?.includes(6)">
                  {{
                    `电话号码为空填充为【${formModel.actionParams.receivePhone?.defaultPhoneNumber}】`
                  }}
                </div>
                <div class="checkItem" v-if="formModel.telephoneAnalysis?.includes(6)">
                  {{
                    `指定【${formModel.actionParams.receivePhone?.replaceStr?.replaceStrX}】替换为【${formModel.actionParams.receivePhone?.replaceStr?.replaceStrY}】`
                  }}
                </div>
              </div>
              <div v-else class="houseNumber bg3"><div class="checkItem">-</div></div>
            ...
            <div class="padb20 d-flex">
              <div class="noshrink tips">收货邮编处理:</div>
              <div class="houseNumber bg3" v-if="formModel.postalCodeCheck">
                <div class="checkItem" v-if="formModel.postalCodeAnalysis?.includes(1)">
                  {{
                    `过滤特殊字符【${formModel.actionParams.receivePostalCode?.filterSpecialStrList}】`
                  }}
                </div>
                <div class="checkItem" v-if="formModel.postalCodeAnalysis?.includes(2)">
                  {{ `去除扩展邮编（仅美国）` }}
                </div>
                <div class="checkItem" v-if="formModel.postalCodeAnalysis?.includes(3)">
                  {{
                    `邮编为空填充为【${formModel.actionParams.receivePostalCode?.defaultPostalCode}】`
                  }}
                </div>
              </div>
              <div v-else class="houseNumber bg3"><div class="checkItem">-</div></div>
            ...
            <div class="padb20 d-flex">
              <div class="noshrink tips">收货姓名处理:</div>
              <div class="houseNumber bg3" v-if="formModel.nameCheck">
                <div class="checkItem" v-if="formModel.nameAnalysis?.includes(1)">
                  {{
                    `过滤特殊字符【${formModel.actionParams.receiveName?.filterSpecialStrList}】`
                  }}
                </div>
                <div
                  class="checkItem"
                ...
                <div class="checkItem" v-if="formModel.nameAnalysis?.includes(3)">
                  用买家姓名替换收货姓名
                </div>
                <div class="checkItem" v-if="formModel.nameAnalysis?.includes(4)">
                  {{ `从第【${formModel.actionParams.receiveName?.subBeginIndex}】个字符开始截取` }}
                </div>
              </div>
              <div v-else class="houseNumber bg3"><div class="checkItem">-</div></div>
            </div>
            ...
            <div class="padb20 d-flex">
              <div class="noshrink tips">收货邮箱处理:</div>
              <div class="houseNumber bg3" v-if="formModel.postalEmailCheck">
                <div class="checkItem">
                  {{
                    formModel.actionParams.receiveEmail.defaultEmail
                      ? `邮箱为空填充为【${formModel.actionParams.receiveEmail.defaultEmail}】`
                      : '-'
                  }}
                </div>
              </div>
...
<script setup lang="ts" name="shippingRuleDetail">
  ...
  const save = async () => {
    ...
    Object.keys(selectedFields).forEach((key) => {
      // console.log(key, selectedFields[key]);
