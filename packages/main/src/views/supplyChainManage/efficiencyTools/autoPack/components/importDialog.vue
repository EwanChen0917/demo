<template>
  <el-dialog title="添加自动装箱" v-model="visibility" width="600px" :before-close="close">
...
<script setup lang="ts">
  ...
  const downloadTemplate = async () => {
    ...
    const res = await erpApi.luteosErpPlanTurnoverQueryuploadAutoPackTemplate().finally(() => {
  ...
  const confirmUpload = async () => {
    await erpApi.luteosErpPlanTurnoverUploadAutoPack({
