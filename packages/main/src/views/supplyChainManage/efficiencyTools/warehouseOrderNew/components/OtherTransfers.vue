  ...
  >
    ...
    <KeenCard title="收件人信息">
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="地址2" prop="recipientInfo.address2">
                ...
                <template #default="{ row, $index }">
                  ...
                  >
                    <KeenFileUpload
                      ...
                      manual
                      @change="
                        formRef.validateField(`warehouseService.packingList.${$index}.shipmentFile`)
                      "
                      @success="
                        (val, file) => {
                          handleUpload(val, file, row, 'shipmentFile', 'SHIPMENT_LABEL_ATTACHMENT');
                          formRef.validateField(
                            `warehouseService.packingList.${$index}.shipmentFile`
                          );
                        }
                      "
                    >
                      <el-button link type="primary" :disabled="isDetail" :underline="false">
                        上传文件
                      </el-button>
                    </KeenFileUpload>
                    <el-icon
                      class="pdf-view-icon"
                      v-if="row.shipmentFile.length && row.shipmentFile[0]?.fileUrl"
                      @click="handlePreviewFile(row.shipmentFile[0])"
                    >
                      <View />
                    </el-icon>
                  </el-form-item>
                ...
                <template #default="{ row, $index }">
                  ...
                  >
                    <KeenFileUpload
                      ...
                      @change="
                        formRef.validateField(
                          `warehouseService.packingList.${$index}.logisticsFile`
                        )
                      "
                      @success="
                        (val, file) => {
              ...
              <template #append>
                <div class="d-flex justify-content-between">
                  <el-space>
                    ...
                    <!--                    <ExportBtn
                      :service="omsApi.omsWarehouseOrderExportGcWarehouseService"
                ...
                <template #default="{ row, $index }">
                  ...
                  >
                    <KeenFileUpload
                      ...
                      accept=".pdf"
                      :overflowHidden="false"
                      ref="rebrandSkuFileUpload"
                      @change="
                        formRef.validateField(`warehouseService.itemList.${$index}.rebrandSkuFile`)
                      "
                      @success="
                        (val, file) => {
                          handleUpload(val, file, row, 'rebrandSkuFile', 'REBRAND_SKU_ATTACHMENT');
                          formRef.validateField(
                            `warehouseService.itemList.${$index}.rebrandSkuFile`
                          );
                        }
                      "
                    >
...
<script setup lang="ts">
  ...
  const rules = reactive({
    ...
    recipientInfo: {
      ...
      address1: [
        { required: true, message: '请输入地址1', trigger: ['blur', 'change'] },
        { max: 50, message: '不能超过50个字符', trigger: ['blur', 'change'] },
      ],
      address2: [{ max: 50, message: '不能超过50个字符', trigger: ['blur', 'change'] }],
  ...
  const save = async () => {
    try {
      ...
      await omsApi.omsWarehouseOrderPushGcTransferSlipOrder({
        code: props.code,
        ...form.value,
        draft: true,
      });
  ...
  const submit = async () => {
    ...
    if (
      ...
      form.value?.warehouseService?.packingList.length === 0
    ) {
      ElMessage.warning('请填写装箱信息');
      return;
    }
    try {
  ...
  const rebrandSkuFileUpload = ref();
  const handleUpload = async (val, file, row, type, gcFileType) => {
    ...
    try {
      ...
      // 上传谷仓附件
      const res = await omsApi.omsWarehouseOrderUploadGuCangFile({
        fileName: file?.name,
        ossKey: file?.ossKey,
        gcFileType,
      ...
      if (res?.fileId) {
        ...
        ElMessage.warning('上传谷仓附件失败，请重试');
        rebrandSkuFileUpload.value?.clearFile();
      }
      return res?.fileId;
    } catch {
      ElMessage.warning('上传谷仓附件失败，请重试');
  ...
  const handleBatchAdd = async () => {
    ...
    const res = await omsApi.omsWarehouseOrderImportGcWarehouseService({
  ...
  const handleCut = async () => {
    try {
      ...
      const res = await omsApi.omsWarehouseOrderCutGuPdf({
  ...
  const handlePreviewFile = (fileInfo) => {
    // 文件地址
    previewFileUrl.value = fileInfo.fileUrl as string;
  ...
  const handleBatchImportMarkOk = async (data) => {
    ...
    for (const item of form.value.warehouseService?.packingList ?? []) {
  ...
  const downloadTemp = async () => {
    // form.value.warehouseService.itemList = skuList.value;
    filesFormat();
    const res = await omsApi.omsWarehouseOrderExportGcWarehouseService({
      ...form.value.warehouseService,
    });
  ...
  const getDetailData = async () => {
    if (props.code) {
      ...
      if (res?.guCangTransferSlipOrderBean && res?.guCangTransferSlipOrderBean?.type === 2) {
        form.value = cloneDeep(res?.guCangTransferSlipOrderBean);
  ...
  const handleCountryChange = () => {
    if (['US', 'AU'].includes(form.value.recipientInfo.countryCode)) {
      formRef.value?.validateField('recipientInfo.province');
    } else {
