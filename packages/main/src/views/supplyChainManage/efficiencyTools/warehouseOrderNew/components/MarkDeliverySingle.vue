<template>
  <el-dialog :title="title" v-model="visible" @close="close()" align-center>
    <p class="fs-xxl-4">绑定B2B订单和三方仓订单，获取三方仓订单发货状态</p>
    <el-form :model="form" ref="formRef">
      <el-form-item
        label="三方仓单号："
        prop="warehouseOrderNo"
        :rules="[{ required: true, message: '三方仓单号必填', trigger: ['blur', 'change'] }]"
      >
...
<script setup lang="ts">
  ...
  const confirm = async () => {
    ...
    if (validateResult) {
      loading.value = true;
      await omsApi
        .omsWarehouseOrderMarkShipped({
          code: state.code!,
          outboundCode: form.warehouseOrderNo,
        })
        .finally(() => {
          loading.value = false;
        });
