<template>
  <el-dialog title="上传销售预测" v-model="visibility" width="600px" :before-close="close">
...
<script setup lang="ts">
  ...
  const downloadTemplate = async () => {
    await erpApi.luteosErpSaleForecastDownTemplate({
      format: 'blob',
    });
  };
  const uploadRef = ref();
  ...
  const confirmUpload = async () => {
    await erpApi.luteosErpSaleForecastUpload({
