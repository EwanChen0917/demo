<template>
  ...
  >
    ...
    <template #buttons>
      <el-button class="button" type="primary" @click="addDialogRef?.open(2, false)">
        添加预测商品
      </el-button>
      <el-button class="button" plain @click="addDialogRef?.open(2, true)">新品预测</el-button>
      <el-button class="button" plain @click="importDialogRef?.open(2)">上传更新</el-button>
      <el-button class="button" plain @click="predictingSnapshotsDialogRef?.open(2)">
        预测历史
      </el-button>
      <el-button
        class="button"
    ...
    >
      ...
      <el-table-column min-width="150" prop="onlineSkuCode" label="在线商品编码" fixed="left">
        <template #header>
          <div class="d-flex align-items-center gap-1">
            <span>在线商品编码</span>
            <el-tooltip
              content="销售渠道面向用户的产品编码；如亚马逊为子ASIN、沃尔玛为item id"
              placement="top"
            >
      ...
      <el-table-column min-width="240" prop="productName" label="产品信息/产品SKU" fixed="left">
        <template #default="{ row }">
          <GoodsInfo
            hide-image
      ...
      <el-table-column min-width="140" prop="skuLabelsDesc" label="在线商品标签">
        <template #default="{ row }">
          <div v-if="row.skuLabelsDesc" class="d-flex flex-wrap gap-1">
            <template v-for="item of row.skuLabelsDesc.split(',')" :key="item">
      ...
      <el-table-column width="160px" prop="modelName" label="型号">
        <template #default="{ row }">
          <div v-if="row.modelName" class="d-flex gap-2">
      ...
      <el-table-column min-width="200" prop="shop" label="店铺" />
      ...
      <el-table-column min-width="100" prop="country" label="站点">
        <template #default="{ row }">
...
<script lang="ts" setup name="onlinegoods">
  ...
  const searchTypeMap = {
    2: {
      label: '在线商品编码',
      placeholder: '在线商品编码(多个可用空格或逗号分隔)',
    },
    1: {
      label: '产品SKU',
  ...
  const handleActions = (item) => {
    ...
    if (key === 'view') {
      ...
      updateDialogRef.value?.open([row], 2);
    } else if (key === 'history') {
  ...
  const multiUpdate = () => {
    ...
    if (errorRows && errorRows.length) {
      ...
      updateDialogRef.value?.open(multipleSelection.value, 2);
    }
  };
  const handleSuccess = () => {
    tableRef.value.clearSelection();
  ...
  const handleCommand = async (command: string) => {
    switch (command) {
      ...
      case 'batchAddProduct':
        batchImportDialogRef.value?.open(2);
        break;
      case 'batchAddNew':
        batchImportDialogRef.value?.open(2);
        break;
