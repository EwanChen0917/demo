  ...
  <el-form :model="form" ref="formRef" label-width="150px">
    <el-tabs v-model="activated" tab-position="top">
      <el-tab-pane label="订单信息" :name="1">
        <KeenCard title="仓配订单号" position="start">
          <template #operation>
            <span>{{ form.baseInfo?.code }}</span>
            <Tag
              style="margin-left: 12px"
              size="large"
              :color="statusColorMap[form.baseInfo.status]"
            >
      ...
      <el-tab-pane label="装板信息" :name="7">
        <BumperInfo
          :orderCode="form.baseInfo.serviceCode"
          ref="bumperInfoRef"
...
<script setup lang="ts" name="warehouseOrderDetail">
  import { erpApi } from '@/api';
  import * as swal from '@/utils/swal';
  import axios from 'axios';
  import { ElMessage } from 'element-plus';
  import FBAPush from '@/views/supplyChainManage/efficiencyTools/warehouseOrder/components/FBAPush.vue';
  import OtherTransfers from '@/views/supplyChainManage/efficiencyTools/warehouseOrder/components/OtherTransfers.vue';
  import BumperInfo from '@/views/supplyChainManage/efficiencyTools/warehouseOrder/components/BumperInfo.vue';
  import LogisticsTrajectory from '@/views/supplyChainManage/efficiencyTools/warehouseOrder/components/LogisticsTrajectory.vue';
  ...
  const bumperInfoRef = ref();
  const logisticsTrajectoryRef = ref();
  ...
  getDetailData();
  // 手动同步装板&物流轨迹
  const refreshGcInfo = async () => {
    const { baseInfo = {}, orderDetailResp = {} } = form.value;
    const { serviceCode } = baseInfo;
    const { customerCode } = orderDetailResp.customerInfoBean ?? {};

    if (!serviceCode || !customerCode) {
      ElMessage.error('订单号或客户号不能为空');
      return;
    }
    try {
      await erpApi.luteosErpWarehouseOrderSyncGcOrderInfo({
        orderCode: serviceCode,
        customerCode,
      });
      if (bumperInfoRef.value) {
        bumperInfoRef.value.queryDeliveryOrderList();
        bumperInfoRef.value.queryBumperList();
      }
      if (logisticsTrajectoryRef.value) {
        logisticsTrajectoryRef.value.queryLogisticsTrajectoryList();
      }
    } catch (error) {}
  };
  ...
  const handleCutAtt = async (row) => {
    ...
    if (isConfirm) {
      await erpApi.luteosErpWarehouseOrderCutAtt({
