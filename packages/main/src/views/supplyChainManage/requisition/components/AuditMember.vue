...
<script setup lang="ts">
  import { memberApi, erpApi } from '@/api';
  ...
  const props = withDefaults(
    ...
    { deptCode: 'all_dept', searchChildDept: true, teleported: false, clearable: true }
  );
  ...
  const getOperatorOptions = async () => {
    ...
    if (props.title && props.title === 'GTM组负责人') {
      ...
      res = props.title
        ? await erpApi.luteosErpMemberQueryOperationMemberList({
            operatorMemberCodeList: ['region_market_person', 'region_channel_person'],
            searchOperatorCode: true,
          })
    ...
    if (props.deptCode === 'region_market_person' || props.deptCode === 'region_channel_person') {
      operatorOptions.value = res.memberListMap[props.deptCode].map((item) => {
        return {
          ...item,
  ...
  watchEffect(() => {
    if (operatorOptions.value?.length > 0 && props.modelValue) {
      ...
      if (typeof props.modelValue === 'string' && !memberCodeArr.includes(props.modelValue)) {
        // console.log('人员不存在', props.modelValue);
