<template>
  <el-drawer
    v-model="visible"
    :size="1200"
    title="新增领用"
    :close-on-click-modal="false"
    :close-on-press-escape="false"
    :destroy-on-close="true"
    :append-to-body="true"
    class="custom-drawer"
    :before-close="onBeforeClose"
  >
    <AddDrawerContent ref="contentRef" @close="visible = false" />
    <template #footer>
      <div>
        <el-button type="primary" @click="handleSave('autoSubmit')">提交申请</el-button>
        <el-button type="primary" plain @click="handleSave('submit')">汇总发起申请</el-button>
        <el-button
          @click="
            onBeforeClose((flag) => {
              visible = flag;
            })
          "
        >
          取消
        </el-button>
        <el-button @click="handleSave('draft')">保存草稿</el-button>
      </div>
    </template>
  </el-drawer>
</template>
<script setup lang="ts">
  import { ElMessageBox } from 'element-plus';
  import AddDrawerContent from '../add.vue';

  const emit = defineEmits<{
    success: [];
  }>();
  const visible = ref(false);
  const code = ref<string>();
  const contentRef = ref<InstanceType<typeof AddDrawerContent>>();

  const open = (payload?: { code?: typeof code.value }) => {
    code.value = payload?.code;
    visible.value = true;
    if (code.value) {
      nextTick(() => {
        contentRef.value?.init(code.value);
      });
    }
  };
  defineExpose({
    open,
  });

  type submitType = 'draft' | 'submit' | 'autoSubmit';
  const handleSave = async (action: submitType) => {
    const result = await contentRef.value?.handleSave(action);
    if (action === 'draft') return;
    visible.value = !result;
    if (result) {
      emit('success');
    }
  };
  const onBeforeClose = async (done) => {
    const flag = await ElMessageBox.confirm('是否保存草稿？', '提示', {
      confirmButtonText: '保存并关闭',
      cancelButtonText: '取消',
    }).catch(() => false);
    if (flag) {
      await handleSave('draft');
      emit('success');
    }
    done(false);
  };
</script>
