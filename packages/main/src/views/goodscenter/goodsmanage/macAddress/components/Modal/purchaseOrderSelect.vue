    ...
    >
      <template #search>
        <el-input
          ...
          placeholder="请输入PO单号/产品名称/供应链SKU/供应商"
          style="width: 350px"
        >
          <template #prefix>
            <SvgIconSearch />
      ...
      >
        <el-table-column width="55" align="center" v-if="selectOne">
          <template #default="scope">
            <el-radio
              v-model="currentRow"
              :label="scope.row"
              :disabled="scope.row.poCode && selectedRow?.poCode === scope.row.poCode"
            >
              {{}}
            </el-radio>
          </template>
        ...
        <el-table-column label="PO采购订单" prop="poCode" min-width="180" />
...
<script lang="ts" setup>
  ...
  const runQuery = async () => {
    try {
      ...
      if (res) {
        dataList.value = res?.purchaseOrderList || [];
