<template>
  ...
  >
    <template #search>
      <el-space>
        <el-input
          ...
          placeholder="请输入ItemId/MSKU/wpid/upc/gtin"
          clearable
          style="width: 300px"
        >
          <template #prefix>
    ...
    <template #filters>
      ...
      <OnlineGoodsLabel v-model="search.tagList" clearable multiple platform="walmart" />
    </template>
    <template #buttons>
      <el-button
        type="primary"
    ...
    >
      ...
      <!--      <el-table-column label="发布日期" prop="listingTime" min-width="110">
        <template #default="{ row }">
...
<script setup lang="ts" name="walmartOnline">
  ...
  const tabStatusList = [
    ...
    { status: 'ARCHIVED', code: 'archivedCount', statusName: '归档', count: '' },
    { status: 'RETIRED', code: 'retiredCount', statusName: '退市', count: '' },
    { status: 'ERROR', code: 'errorCount', statusName: '异常', count: '' },
  ];
  ...
  const {
    ...
    ProductApi.LuteosProductWalmartListingQueryProductListingPage.RequestBody,
    ProductApi.LuteosProductWalmartListingQueryProductListingPage.ResponseBody
  >({
    searchDefaults: {
      keyword: undefined,
      shopList: undefined,
