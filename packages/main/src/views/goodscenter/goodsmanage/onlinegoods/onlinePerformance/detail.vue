<template>
  <div class="d-flex w-100 gap-5 flex-wrap">
    <div style="width: 900px; flex-grow: 1">
      <KeenCard title="渠道信息" class="w-100 h-400px">
        <div class="d-flex">
          <KeenImagePreview
              ...
              <el-row>
                <el-col :span="24">
                  <el-form-item label="店铺SKU：">
                    <span>{{ detailInfo?.sellerSku }}</span>
                  </el-form-item>
                </el-col>
              </el-row>
              ...
              <el-row>
                <el-col :span="12">
                  <el-form-item label="上线时间：">
                    <span>{{ detailInfo?.upLineTime }}</span>
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="首单时间：">
                    <span>{{ detailInfo?.firstOrderTime }}</span>
                  </el-form-item>
                </el-col>
              </el-row>
            </el-form>
      ...
      <KeenCard title="销售表现" :margin-bottom="0" class="w-100" style="height: 760px">
        ...
        <div v-else class="empty w-100 h-600px">
          <EmptyAnimation name="listEmpty" :width="128" :height="128" />
          <span>暂无数据</span>
        </div>
      </KeenCard>
    </div>
    <div v-if="isAmazon" class="d-flex flex-wrap w-400px" style="flex-grow: 1">
      <KeenCard class="w-100 h-400px">
        ...
        <RankChart
          ...
          "
        />
        <div v-else class="empty w-100 h-300px">
          <EmptyAnimation name="listEmpty" :width="128" :height="128" />
          <span>暂无数据</span>
        </div>
      </KeenCard>
      <KeenCard :margin-bottom="0" class="w-100" style="height: 760px">
...
<script setup lang="ts" name="onlineGoodsPerformanceDetail">
  import RankChart from '@/views/goodscenter/goodsmanage/onlinegoods/onlinePerformance/components/rankChart.vue';
  import { productApi } from '@/api';
  import dayjs from 'dayjs';
  import SaleChart from '@/views/goodscenter/goodsmanage/onlinegoods/onlinePerformance/components/saleChart.vue';
  ...
  const { site, channel, shop, asin, sellerSku, skuCode } = route.query;
  ...
  const queryStatisticsDetail = async () => {
    const res = await productApi.luteosProductShowSearchStatisticsDetail({
      site,
      channel,
      shop,
      asin,
