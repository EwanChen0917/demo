...
<script setup lang="ts">
  ...
  const downloadTemplate = async () => {
    ...
    const res: any = await commonApi
      .luteosCommonFileQueryImportTemplate({ importScene: props.importScene ?? 'category_level5' })
      .finally(() => {
  ...
  const confirmUpload = async () => {
    await commonApi.luteosCommonFileImportData({
      fileName: file.value[0].name,
      key: file.value[0].ossKey,
      importScene: props.importScene ?? 'category_level5',
    });
    file.value = [];
    emits('success');
    close();
