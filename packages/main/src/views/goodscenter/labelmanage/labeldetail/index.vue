                ...
                <div class="card card-flush py-4">
                  ...
                  <div class="card-body pt-0">
                    ...
                    >
                      ...
                      <el-form-item label="关联品线：" v-else-if="isGtm">
                        <el-select
                          v-model="form.productLineCodeList"
                          filterable
                          multiple
                          placeholder="请选择产品品线"
                        >
                          <el-option
                            v-for="type in productLineList"
                            :value="type.code"
                            :label="type.productLine"
                            :key="type.code"
                          />
                        </el-select>
                      </el-form-item>
...
<script setup lang="ts" name="labeldetail">
  ...
  const getGtm = async () => {
    const res = await productApi.luteosProductLineQueryList({ pageNum: 1, pageSize: 100 });
  ...
  watchEffect(() => {
    ...
    if (tag?.tagBean) {
      ...
      form.categoryCode = tagBean.categoryCode;
    }
    ...
    if (tag?.productLineBeanList) {
      const { productLineBeanList } = tag;
      form.productLineCodeList = productLineBeanList?.map((item) => item.code);
    }
  });
  ...
  const isGtm = computed(() => {
    return form.tagType === 'gtm';
  });
  ...
  const handleSaveSubmit = async () => {
    ...
    if (valid) {
      try {
        ...
        const { tagCode, tagName, tagRemark, tagType, categoryCode } = form;
        ...
        if (form.productLineCodeList?.length) {
          params.itemCodeList = form.productLineCodeList;
        }
