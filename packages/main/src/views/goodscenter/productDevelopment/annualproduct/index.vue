<template>
  <div class="card-row">
      ...
      <div
        ...
        :class="{
          active:
            ...
            !searchParams.productStatusList?.length &&
        ...
        <template v-if="item?.children?.length">
          <div v-for="child in item.children" :key="child.label" class="d-flex">
            <div class="child-wrap">
              <!-- 如期延期 -->
              <b
                class="child-count-desc"
                @click.stop.prevent="searchByDelayStatus(item.childKey, child.status)"
              >
                {{ child.label }}
              </b>
            </div>
    ...
    <template v-for="item in statusList" :key="item.key">
      <div
        ...
        :class="{
          active: !searchParams.productStage && searchParams.productStatusList.includes(item.key),
        }"
      >
        <div class="card-item-title">
          <div class="d-flex align-items-center gap-1">
  ...
  >
    ...
    <template #filters>
      ...
      <el-select
        v-model="searchParams.productStatusList"
        placeholder="状态"
        :teleported="false"
        filterable
        clearable
      ...
      <el-date-picker
        ...
        type="daterange"
        start-placeholder="预计上架开始"
        end-placeholder="预计上架结束"
        value-format="YYYY-MM-DD"
        @change="search"
      />
      <el-date-picker
        style="width: 320px; flex-grow: 0"
        v-model="searchParams.planStockTime"
        type="daterange"
        start-placeholder="预计中仓入库开始"
        end-placeholder="预计中仓入库结束"
        value-format="YYYY-MM-DD"
        @change="search"
      />
      ...
      >
        <div class="h-xl-100">
          ...
          <div class="py-5 h-xl-100">
            ...
            >
              <div class="mb-5">
                <div class="form-label fs-5 fw-semibold mb-3">项目经理:</div>
                <el-select
              ...
              <div class="mb-5">
                <div class="form-label fs-5 fw-semibold mb-3">产品经理:</div>
                <OperatorSelect v-model="searchParams.productOwnerCode" @change="search" />
              </div>
              <div class="mb-5">
                <div class="form-label fs-5 fw-semibold mb-3">产品级别:</div>
                <el-select
            ...
            <div
              ...
              style="padding-left: 1.75rem; padding-right: 1.75rem; gap: 8px;"
            >
  ...
  <!--  <BatchImportModal
    ...
    @close="batchUpdate"
  />-->
  <ImportDialog v-if="uploadVisible" :visible="uploadVisible" @close="batchUpdate" />
</template>
...
<script setup lang="ts" name="annualproduct">
  ...
  import ImportDialog from '@/views/goodscenter/productDevelopment/annualproduct/Modal/importDialog.vue';
  ...
  const stageList = ref([
    ...
    },
    {
      key: '3',
      label: '入中仓',
      tooltip: '计划中仓入库时间≤当前时间＜计划上架时间的项目数量',
      count: 'stockStageCount',
      rate: 'stockStagePercentage',
      color: 'text-primary',
      childKey: 'stockStatus',
      children: [
        {
          label: '如期',
  ...
  // 根据如期延期状态查询
  const searchByDelayStatus = (status: string, value: number) => {
    searchParams.value = {
      productStage: '',
      productStatusList: [],
      productName: '',
  ...
  const getProductData = () => {
    ...
    const data: any = {
      ...
      productStatusList: searchParams.value.productStatusList,
      ...
      listingTimeStart: searchParams.value.listingTime?.[0],
      listingTimeEnd: searchParams.value.listingTime?.[1],
      planStockTimeStart: searchParams.value.planStockTime?.[0],
      planStockTimeEnd: searchParams.value.planStockTime?.[1],
      stockStatus: searchParams.value.stockStatus,
      listingStatus: searchParams.value.listingStatus,
    };
  ...
  const handleSubmit = async () => {
    if (curItemInfo.value.type === 1) {
      const data: any = {
        ...
        updateTypeList: curItemInfo.value.updateTypeList,
      ...
      const data: any = {
        ...
        productOwnerCode: curItemInfo.value.productOwnerCode.join('、'),
        projectOwner: curItemInfo.value.projectOwner.join('、'),
        ...
        firstDeliveryQuantity: curItemInfo.value.firstDeliveryQuantity,
        firstDeliveryPlan: curItemInfo.value.firstDeliveryPlan,
        planYear: curItemInfo.value.planYear,
      };
  ...
  // 暂停操作
  const pause = async () => {
    if (!productCodeList.value.length) return ElMessage.warning('请先勾选数据');
    const isConfirmed = await swal.confirm('确定要暂停吗？');
    if (!isConfirmed) return;
    await pdtApi.luteosPdtOperatePdt({
      operateType: 'disable',
      productCodeList: productCodeList.value,
    });
    ElMessage.success('暂停成功');
    tableRef.value?.EleTableRef?.innerTable?.clearSelection();
    getProductData();
  };
  // 启用操作
  const enable = async () => {
  ...
  : '', */
      预计上架时间: searchParams.value.listingTime?.length
        ? `${searchParams.value.listingTime?.[0]} - ${searchParams.value.listingTime?.[1]}`
        : '',
