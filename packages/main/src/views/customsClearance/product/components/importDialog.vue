<template>
  <el-dialog title="导入报关产品" v-model="visibility" width="600px" :before-close="close">
...
<script setup lang="ts">
  ...
  const downloadTemplate = async () => {
    ...
    const res = await erpApi.luteosErpCustomsDownProductTemplate().finally(() => {
  ...
  const confirmUpload = async () => {
    await erpApi.luteosErpCustomsImportProduct({
