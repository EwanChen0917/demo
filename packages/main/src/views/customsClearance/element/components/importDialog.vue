<template>
  <el-dialog title="导入报关要素" v-model="visibility" width="600px" :before-close="close">
...
<script setup lang="ts">
  ...
  const downloadTemplate = async () => {
    ...
    const res = await erpApi.luteosErpCustomsDownElmentTemplate().finally(() => {
  ...
  const confirmUpload = async () => {
    await erpApi.luteosErpCustomsImportElement({
