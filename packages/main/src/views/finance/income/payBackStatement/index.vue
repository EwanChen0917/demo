<template>
  <KeenCard title="推送任务进度" position="start" grow class="push-progress-bar">
    ...
    <template v-if="showFlag">
      <div class="tip">显示今日最近9条任务进度，更多任务请查看历史记录</div>
      <div class="task-list" v-loading="pushListLoading">
  ...
  >
    <template #search>
      <el-input v-model="search.keyword" placeholder="收款单号" clearable>
        <template #prefix>
          <SvgIconSearch />
        </template>
      </el-input>
    ...
    <template #buttons>
      ...
      <ExportBtn
        :service="financeSystemApi.financeIncomePaymentConfirmationOrderExportQueryList"
        :params="{
          ...omit(search, 'time'),
    ...
    <template #tools-prepend>
      <el-button
        type="primary"
        plain
        :loading="store.taskMap[refreshType.收款单].apiSync"
        @click="handleRefresh"
      >
        更新推送状态
      </el-button>
    ...
    >
      ...
      <el-table-column label="收款单号" prop="code" min-width="140" fixed />
      ...
      <el-table-column label="回款时间" prop="paymentConfirmationTime" />
...
<script setup lang="ts" name="payBackStatement">
  ...
  const {
    ...
    FinanceSystemApi.FinanceIncomePaymentConfirmationOrderExportQueryList.RequestBody,
    FinanceSystemApi.FinanceIncomePaymentConfirmationOrderExportQueryList.ResponseBody
  >({
    searchDefaults: {
      keyword: undefined,
      shopList: undefined,
    ...
    pageSize: PAGE_SIZE,
    service: financeSystemApi.financeIncomePaymentConfirmationOrderQueryList,
  });
  ...
  const handleActions = async (item) => {
    ...
    switch (key) {
      ...
      case 'push': {
        ...
        await financeSystemApi.financeIncomePaymentConfirmationOrderPush({
          code: row?.code,
  ...
  const queryPushBatchList = async () => {
    try {
      ...
      const res = await financeSystemApi.financeIncomePaymentConfirmationOrderQueryPushBatchList({
        pageSize: 9,
        pageNum: 1,
      });
  ...
  const batchPush = async (params, tip) => {
    try {
      ...
      const res = await financeSystemApi.financeIncomePaymentConfirmationOrderPushBatch(params);
  ...
  const batchDelete = async (params, tip) => {
    try {
      ...
      const res = await financeSystemApi.financeIncomePaymentConfirmationOrderDeletedBatch(params);
      if (res?.failList?.length) {
        failDialogRef.value?.open(res?.failList);
      } else {
        ElMessage.success(`${tip}成功`);
  ...
  const markPushStatus = async (tip, status, type) => {
    try {
      ...
      const res = await financeSystemApi.financeIncomePaymentConfirmationOrderMarkPushStatus(param);
      if (res?.failList?.length) {
        failDialogRef.value?.open(res?.failList);
      } else {
        ElMessage.success(`${tip}成功`);
  ...
  const handleRefresh = async () => {
    await financeSystemApi.financeBasicPlatformRefresh({
      modelCode: 'income_payment_confirmation_order',
      param: search,
      sleepTime: 0,
    });
    store.taskMap[refreshType.收款单].apiSync = true;
    financeBudgetRefresh(user.token as string, refreshType.收款单);
  };
  watch(
    () => store.taskMap[refreshType.收款单].apiSync,
    (val) => {
      if (!val) refreshList();
    }
  );
