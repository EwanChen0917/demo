<template>
  <KeenCard title="推送任务进度" position="start" grow class="push-progress-bar">
    <template #operation>
      <div class="d-flex justify-content-between">
        ...
        <div class="d-flex">
          <div class="refresh" @click="queryPushBatchList">
            <i class="iconfont icon-shujutongbu1" style="font-weight: bolder"></i>
            <el-button type="primary" link class="mr_12">刷新</el-button>
            <div
              v-show="!showFlag"
  ...
  >
    ...
    <template #buttons>
      ...
      <el-dropdown @command="handleDeleteCommand">
        <el-button type="primary" :loading="batchDeleteLoading">
          批量删除
          <el-icon class="el-icon--right">
            <arrow-down />
          </el-icon>
      ...
      <ExportBtn
        :service="financeSystemApi.financePlatformOrderExport"
        :params="{
          ...omit(search, 'time'),
    ...
    <template #tools-prepend>
      <el-button
        type="primary"
        plain
        :loading="store.taskMap[refreshType.平台库存单].apiSync"
        @click="handleRefresh"
      >
        更新推送状态
      </el-button>
    </template>
    <div class="statistics statistics-in-list">
      <div class="statistics-item">
        <span class="cycle" style="background: #b6b6b6"></span>
    ...
    >
      ...
      <el-table-column label="单据时间" prop="orderTime" min-width="140" />
      <el-table-column label="备注" prop="errorRemark" min-width="150">
      ...
      <el-table-column label="推送状态" prop="pushStatusDesc">
        <template #default="{ row }">
          <Tag :color="pushColorMap[row?.pushStatus]">
            {{ pushStatusMapList[row?.pushStatus] }}
          </Tag>
        </template>
      </el-table-column>
          ...
          <KeenActions
            ...
            :actions="[
              ...
              },
              {
                ...
                key: 'push',
                type: 'primary',
                hide: row?.pushStatus !== pushStatus.未推送 || row?.type == 'ending-inventory',
                permissionCode: '',
                row,
              },
              {
                ...
                key: 'push',
                type: 'primary',
                hide: row?.pushStatus !== pushStatus.推送失败 || row?.type == 'ending-inventory',
                permissionCode: '',
                row,
              },
            ]"
...
<script setup lang="ts" name="financeBillsStatement">
  ...
  import {
    ...
    taskStatus,
    taskStatusMap,
  } from '@/views/finance/income/financeBills/enum';
  ...
  import { ruleType, financeBillsRuleTypeMap } from '@/views/finance/income/incomeRule/enum';
  ...
  const {
    ...
    FinanceSystemApi.FinancePlatformOrderQueryList.RequestBody,
    FinanceSystemApi.FinancePlatformOrderQueryList.ResponseBody
  >({
    searchDefaults: {
    ...
    pageSize: PAGE_SIZE,
    service: financeSystemApi.financePlatformOrderQueryList,
  });
  ...
  const handleActions = async (item) => {
    ...
    switch (key) {
      ...
      case 'push': {
        ...
        await financeSystemApi.financePlatformOrderPushBatch({
          codes: [row?.code],
          ruleTypeList: [row?.type],
  ...
  const queryPushBatchList = async () => {
    try {
      ...
      const res = await financeSystemApi.financePlatformOrderQueryPushBatchList({
        pageSize: 9,
        pageNum: 1,
      });
  ...
  const batchPush = async (params, tip) => {
    try {
      ...
      const res = await financeSystemApi.financePlatformOrderPushBatch(params);
  ...
  const batchDelete = async (params, tip) => {
    try {
      ...
      const res = await financeSystemApi.financePlatformOrderDeletedBatch(params);
      if (res?.failList?.length) {
        failDialogRef.value?.open(res?.failList);
      } else {
        ElMessage.success(`${tip}成功`);
  ...
  const markPushStatus = async (tip, status, type) => {
    try {
      ...
      const res = await financeSystemApi.financePlatformOrderMarkPushStatus(param);
      if (res?.failList?.length) {
        failDialogRef.value?.open(res?.failList);
      } else {
        ElMessage.success(`${tip}成功`);
  ...
  const handleRefresh = async () => {
    await financeSystemApi.financeBasicPlatformRefresh({
      modelCode: 'inventory_report_order',
      param: search,
      sleepTime: 0,
    });
    store.taskMap[refreshType.平台库存单].apiSync = true;
    financeBudgetRefresh(user.token as string, refreshType.平台库存单);
  };
  watch(
    () => store.taskMap[refreshType.平台库存单].apiSync,
    (val) => {
      if (!val) refreshList();
    }
  );
