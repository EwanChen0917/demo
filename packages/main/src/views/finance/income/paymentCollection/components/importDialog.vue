<template>
  <el-dialog title="导入收款记录" v-model="visibility" width="600px" :before-close="close">
...
<script setup lang="ts">
  ...
  const downloadTemplate = async () => {
    ...
    const res = await financeSystemApi.financePaymentCollectionDownloadTemplate().finally(() => {
  ...
  const confirmUpload = async () => {
    await financeSystemApi.financePaymentCollectionImportData({
