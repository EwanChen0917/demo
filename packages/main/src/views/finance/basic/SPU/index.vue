<template>
  <div>
    ...
    >
      <template #search>
        <el-input v-model="search.keyword" placeholder="编码/名称/金蝶编码/分贝通编码" clearable>
          <template #prefix>
            <SvgIconSearch />
          </template>
        </el-input>
      ...
      >
        <el-table-column label="编码" prop="productSpu" min-width="90" />
...
<script setup lang="ts" name="financeSPU">
  ...
  const {
    ...
    FinanceSystemContracts.SpuListQueryResp
  >({
    searchDefaults: {
      keyword: undefined,
    ...
    pageSize: PAGE_SIZE,
    service: financeSystemApi.financeBasicQuerySpuList,
  });
  watch(listData, () => {
