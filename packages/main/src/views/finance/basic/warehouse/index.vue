<template>
  ...
  >
    ...
    >
      ...
      <el-table-column label="积加仓库ID" prop="jjWarehouseId" />
      <el-table-column label="积加仓库名称" prop="jjWarehouseName" min-width="100" />
      <el-table-column label="服务商" prop="serviceProviderName" />
      <el-table-column label="地区" prop="countryCode" />
      ...
      <el-table-column label="录入来源" prop="inputSourceDesc" min-width="120" />
...
<script setup lang="ts" name="financeWarehouse">
  ...
  const {
    ...
    FinanceSystemApi.FinanceBasicQueryWarehouseList.RequestBody,
    FinanceSystemApi.FinanceBasicQueryWarehouseList.ResponseBody
  >({
    searchDefaults: {
      keyword: undefined,
    ...
    pageSize: PAGE_SIZE,
    service: financeSystemApi.financeBasicQueryWarehouseList,
  });
  ...
  const handleActions = async (item) => {
    ...
    if (key === 'push') {
      ...
      const res = await financeSystemApi
        .financeBasicSyncKingdeeBill({ billNo: row?.warehouseCode, billType: 6 })
        .catch(() => {
          runQuery();
        });
      if (res) {
  ...
  const changeKingdeeNumber = async (row) => {
    const param = {
      ...
      billType: 6, // -1不存在，1客户，2供应商，3产品资料，4员工，5部门，6仓库
      billNo: row.warehouseCode,
    };
