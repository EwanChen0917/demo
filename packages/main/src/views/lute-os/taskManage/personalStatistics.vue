  ...
  <KeenList>
    ...
    <template #search>
      <div class="d-flex gap-2 align-items-center flex-wrap">
        ...
        />
        <el-checkbox
          v-model="search.mineHandleFlag"
          label="我执行的"
          :true-value="1"
          :false-value="0"
          @change="handleMineHandleChange"
        />
        <el-checkbox
          v-model="search.mineJoinHandleFlag"
          label="我共同执行的"
          :true-value="1"
          :false-value="0"
          @change="handleMineJoinHandleChange"
        />
...
<script setup lang="ts">
  ...
  const reset = () => {
    resetSearch();
    search.mineHandleFlag = 0;
    search.mineJoinHandleFlag = 0;
    search.status = 0;
  };

  function handleMineHandleChange(val) {
    if (val === 1) {
      search.mineJoinHandleFlag = 0;
    }
  }
  function handleMineJoinHandleChange(val) {
    if (val === 1) {
      search.mineHandleFlag = 0;
    }
  }
