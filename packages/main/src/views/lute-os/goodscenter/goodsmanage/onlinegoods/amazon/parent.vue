<template>
  <KeenList>
    ...
    >
      ...
      <el-table-column label="子ASIN" min-width="150">
        <template #default="{ row }">
          <span v-if="row?.asinList?.length === 1">{{ row?.asinList[0] }}</span>
          <el-dropdown trigger="click" v-else-if="row?.asinList?.length > 1">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item v-for="(item, index) in row.asinList" :key="item">
      ...
      <el-table-column label="FNSKU" min-width="150">
        <template #default="{ row }">
          <span v-if="row?.fnSkuList?.length === 1">{{ row?.fnSkuList[0] }}</span>
          <el-dropdown trigger="click" v-else-if="row?.fnSkuList?.length > 1">
            <span class="el-dropdown-link">
              {{ row?.fnSkuList[0] }}
              <el-icon class="el-icon--right"><arrow-down /></el-icon>
            </span>
            <template #dropdown>
              <el-dropdown-menu>
                <el-dropdown-item v-for="(item, index) in row.fnSkuList" :key="item">
                  <span v-if="index > 0">{{ item }}</span>
                </el-dropdown-item>
              </el-dropdown-menu>
            </template>
      ...
      <el-table-column label="运营方式">
        <template #default="{ row }">
          <Tag class="mb-2" v-for="item in row.operationMethodList" :key="item.operationMethod">
            {{ item.operationMethodDesc }}
          </Tag>
        </template>
      </el-table-column>
...
<script setup lang="ts">
  ...
  >({
    ...
    pageSize: PAGE_SIZE,
    service: productApi.luteosProductAmazonListingQueryParentAsinPage,
  });
