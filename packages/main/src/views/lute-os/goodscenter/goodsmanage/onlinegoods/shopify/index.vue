  ...
  <KeenList>
    <template #search>
      <el-space>
        <el-input
          ...
          placeholder="请输入ProductId/variantId/MSKU"
          clearable
          style="width: 275px"
        >
          <template #prefix>
    ...
    <template #filters>
      ...
      <DeptMember
        ...
        deptId="65717209"
      />
      <OnlineGoodsLabel v-model="search.tagList" clearable multiple platform="shopify" />
    </template>
    <template #buttons>
      <el-button type="primary" @click="runQuery">查询</el-button>
      <el-button type="primary" @click="resetSearch">重置</el-button>
      <el-button type="primary" @click="batchEditTag" v-permission="'childEditTag'">
        批量操作
      </el-button>
      <ExportBtn :ignore-app-code="true"
        :service="productApi.luteosProductShopifyListingDownload"
        :params="{
          ...search,
        }"
      >
    ...
    >
      ...
      <el-table-column label="店铺SKU信息" min-width="220">
      ...
      <el-table-column label="产品SKU">
        <template #default="{ row }">
          <div class="fs-7" v-for="(item, index) in row?.skuCodeList" :key="index">
            {{ item }}
          </div>
        </template>
      </el-table-column>
      <el-table-column label="运营" prop="operatorName">
        <template #default="{ row }">
          <div class="fs-7" v-for="(item, index) in row?.operatorList" :key="index">
            {{ item.site }} {{ item.operatorName }}
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="发布日期" prop="publishTime" min-width="110">
        <template #default="{ row }">
          <span class="fs-7">{{ row?.publishTime }}</span>
        </template>
      </el-table-column>
...
<script setup lang="ts" name="shopifyOnline">
  ...
  import EditOperatorDialog from '@/views/lute-os/goodscenter/goodsmanage/onlinegoods/shopify/components/editOperatorDialog.vue';
  ...
  const tabStatusList = [
    ...
    { status: 'active', code: 'activeCount', statusName: '正常', count: '' },
    { status: 'draft', code: 'draftCount', statusName: '草稿', count: '' },
  ];
  ...
  const {
    ...
    ProductApi.LuteosProductShopifyListingQueryProductListingPage.RequestBody,
    ProductApi.LuteosProductShopifyListingQueryProductListingPage.ResponseBody
  >({
    searchDefaults: {
      keyword: undefined,
      shopList: undefined,
  ...
  const handleActions = async (item) => {
    ...
    if (key === 'editLink') {
      childInfo.value = `variantId：${row.variantId}`;
      curRow.value = row;
