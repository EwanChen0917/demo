<template>
  ...
  <KeenList>
    <template #search>
      <el-input v-model="search.saleSkuCode" placeholder="渠道唯一标识/产品SKU/供应链SKU" clearable>
        <template #prefix>
          <SvgIconSearch />
        </template>
      </el-input>
    ...
    <template #buttons>
      ...
      <!--      <ExportBtn
        v-permission="'skurelationExport'"
        :service="productApi.luteosProductSaleSkuExportSaleSku"
        :params="{
          ...search,
    ...
    >
        ...
        <template #default="{ row }">
          <div class="d-flex align-items-center pe-5">
            ...
            <i
              ...
              @click="copyFn(row.saleSkuCode === 'NaN' ? row.msku : row.saleSkuCode, '')"
            ></i>
          </div>
          <div class="d-flex align-items-center pe-5">
            <span class="text-gray-700 fw-bold">
              店铺SKU：{{ row.channel === 'amazon' ? row.msku : row.saleSkuCode }}
            </span>
            <i
              class="fas fa-copy text-hover-primary cursor-pointer"
              @click="copyFn(row.channel === 'amazon' ? row.msku : row.saleSkuCode, '')"
            ></i>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="产品SKU*数量" min-width="190" v-if="search.status !== 1">
        <template #default="{ row }">
          ...
          <div
            ...
            :key="index"
          >
            <el-button type="text" @click="toSkuList(item.skuCode, item.productType)">
              {{ item.skuCode }}
            </el-button>
      ...
      <el-table-column prop="sourceDesc" label="来源"></el-table-column>
...
<script lang="ts" setup name="skurelationnew">
  ...
  import ExportSelectDialog from '@/views/lute-os/goodscenter/goodsmanage/skurelation/components/exportSelectDialog.vue';
  ...
  const querySourceList = () => {
    productApi
      .luteosProductSaleSkuQuerySource({})
      .then((res: any) => {
