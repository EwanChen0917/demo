<template>
  <!--  <KeenToolbarActions>
      <div class="d-flex align-items-center" style="width: 50vw" v-if="form.state === 1">
        <el-icon class="text-gray-400 fs-1">
          <Warning />
        </el-icon>
        <span class="badge badge-light-secondary text-gray-500 fs-3">该产品正在审批中，不可修改</span>
      </div>
    </KeenToolbarActions>-->
  <el-form
    ref="formRef"
      ...
      <template #auto>
        ...
        <KeenCard title="产品经理">
          <el-form-item prop="createBy" label-width="0">
            <OperatorSelect v-model="form.createBy" @change="getChildOperatorOptions" />
          </el-form-item>
        </KeenCard>
      </template>
      <template #fluid>
        <el-tabs v-model="tabCurrent" @tab-Change="tabChange" :before-leave="beforeLeave">
                ...
                <el-form-item label="建议销售价：" required>
                  <div class="w-100 d-flex gap-5">
                    ...
                    <el-form-item prop="currencyCode">
                      <CurrencySelect v-model="form.currencyCode" />
                    </el-form-item>
                  </div>
                </el-form-item>
              </div>
          ...
          <!--          <el-tab-pane label="供应商信息" :name="2" lazy>
                      <KeenCard title="供应商信息">
                        <template #operation>
                          &lt;!&ndash;                <span v-if="isAudit" class="badge badge-light-primary"></span>&ndash;&gt;
                          <span v-if="!isEdit" class="badge badge-light-primary">工单状态：待提交</span>
                          <span
                            v-else-if="form?.productPurchase.approveStatus"
                            :class="['badge', approveStatusMap[form.productPurchase.approveStatus]]"
                          >
                            工单状态：{{ form?.productPurchase.approveStatusDesc || '' }}
                          </span>
                        </template>
                        <PurchaseInfo
                          ref="purchaseInfoFormRef"
          ...
          <!--          <el-tab-pane label="报关/清关信息" :name="3" lazy>
                      <KeenCard title="报关/清关信息">
                        <template #operation>
                          <span v-if="!isEdit" class="badge badge-light-primary">工单状态：待提交</span>
                          <span
                            v-else-if="form?.productDeclare.approveStatus"
                            :class="['badge', approveStatusMap[form.productDeclare.approveStatus]]"
                          >
                            工单状态：{{ form?.productDeclare.approveStatusDesc || '' }}
                          </span>
                        </template>
                        <DeclareInfo
          ...
          <el-tab-pane label="报关/清关信息" :name="3" lazy>
            ...
            >
              <template #tips>
                <div
                  v-if="form.declareApproveStatus === 1"
                  class="d-flex align-items-center text-success fw-bold mb-3 pe-2"
                >
                  <i class="ki-duotone ki-information fs-1 me-1">
                    <i class="path1"></i>
                    <i class="path2"></i>
                  ...
                  <el-button
                    type="text"
                    size="small"
                    @click="toWorkflowDetail(form.declareWorkflowCode)"
                    class="p-0 h-20px"
                  >
                    # {{ form.declareWorkflowCode }}
                  </el-button>
                </div>
              </template>
            </DeclareInfo>
        ...
        <KeenCard title="SKU信息">
          ...
          <el-table :data="form.skuList.filter((item) => item.activeFlag)">
            ...
            <el-table-column label="操作" min-width="30">
              <template #default="scope">
                <el-link type="danger" :underline="false" @click="quitSku(scope.row, scope.$index)">
                  废弃
                </el-link>
              </template>
            </el-table-column>
...
getDetailInfo(productSpu);
  ...
  const setProductDeclareList = () => {
    ...
    const oldList = form.productDeclareList;
    // 为了保持数据的不可变性，首先创建一个新的productDeclareList
    // 使用扩展运算符和filter来确保数据的正确转换和过滤
    // 将新列表赋值给form.productDeclareList
    form.productDeclareList = form.skuList.map((item) => {
      // 查找旧列表中相同的项，以便回显数据
      const same = oldList.find((old) => old.skuCode === item.skuCode);
      // 创建一个新的对象，合并需要的属性
      return {
        ...item,
        ...(same && {
          ...Object.fromEntries(Object.entries(same).filter(([key]) => key !== 'activeFlag')),
        }),
      };
    });
  };
      ...
      switch (tabCurrent.value) {
        case 0:
          tabsStatusMap[tabCurrent.value].valid =
            ...
            (await Promise.all([formRef.value?.validate().catch(() => false), otherFormRef.value?.validate().catch(() => false)]));
          break;
        case 1: {
          tabsStatusMap[tabCurrent.value].valid =
            await specificationsInfoFormRef.value?.formRef?.validate().catch(() => {
          return false; // 校验失败返回 false，不继续执行
        });
          break;
        }
        case 2:
          tabsStatusMap[tabCurrent.value].valid =
            await purchaseInfoFormRef.value?.formRef?.validate().catch(() => {
          return false; // 校验失败返回 false，不继续执行
        });
          break;
        case 3:
                      tabsStatusMap[tabCurrent.value].valid = await declareInfoFormRef.value?.formRef?.validate().catch(() => {
              return false; // 校验失败返回 false，不继续执行
            });
          break;
        default:
