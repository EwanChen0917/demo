<template>
  <KeenList>
    ...
    <el-table ref="tableRef" v-loading="listLoading" :data="listData?.macAddressList || []">
      ...
      <el-table-column label="MAC地址" prop="macAddr" min-width="150" />
      ...
      <el-table-column label="贴片工厂" prop="factoryName" min-width="150">
        <template #default="{ row }">{{ row.factoryCode }}-{{ row.factoryName }}</template>
      </el-table-column>
      <el-table-column label="产品代码" prop="productCode" min-width="120" />
      <el-table-column label="系列" prop="seriesDesc" min-width="120" />
      <el-table-column label="衍生/迭代" prop="deriveDesc" min-width="120" />
      <el-table-column label="状态" prop="statusDesc" min-width="100">
        <template #default="{ row }">
          <Tag :color="row.status === 1 ? 'green' : 'red'">{{ row.statusDesc }}</Tag>
        </template>
      </el-table-column>
      <el-table-column label="开发阶段" prop="developState" min-width="100" />
      <el-table-column label="来源PO" prop="poCode" min-width="180" />
      <el-table-column label="订单行" prop="orderLine" min-width="100" />
      <el-table-column label="供应商" prop="supplierName" min-width="280" />
