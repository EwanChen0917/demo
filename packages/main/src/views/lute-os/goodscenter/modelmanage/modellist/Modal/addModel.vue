<template>
  <el-dialog title="新增型号" :model-value="visible" @close="$emit('close')">
...
<script lang="ts" setup>
  ...
  const form = reactive<ProductApi.LuteosProductModelSaveOrUpdateModel.RequestBody>({
    modelName: '',
    modelRemark: '',
  });
  ...
  const handleFormSubmit = async () => {
    ...
    if (valid) {
      try {
        ...
        const { modelName, modelRemark } = form;

        const res = await productApi.luteosProductModelSaveOrUpdateModel({
          modelName,
          modelRemark,
        });
