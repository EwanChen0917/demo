<template>
  ...
  >
    <KeenList>
      <template #search>
        <el-input v-model="search.fileName" placeholder="用户名称" @change="search">
          <template #prefix>
            <SvgIconSearch />
          </template>
        </el-input>
        ...
        <vxe-column field="name" title="课件名称" min-width="100">
          <template #default="{ row }">
            <div class="d-flex align-items-center gap-2">
              <SvgIcon :icon="getSvgIcon(row)" class="svg-icon svg-icon-1 svg-icon-primary" />
              <!-- 文件夹名称 -->
              <span
                v-if="row.fileType === 1"
              ...
              <!-- 文件名称 -->
              <span v-else class="text-gray-800 text-hover-primary cursor-pointer">
...
<script setup lang="ts">
  ...
  const labelList = ref([]);
  // const queryLabel = () => {
  //   fccApi.luteosFccTagQueryFccLabelList({ pageNum: 1, pageSize: 100 }).then((res) => {
