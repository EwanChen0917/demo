<template>
  <KeenList>
    ...
    <template #buttons>
      ...
      <el-button class="button" type="primary" @click="handleAdd">新增工具</el-button>
    </template>

    <el-table ref="tableRef" v-loading="listLoading" :data="materialList" row-key="materialCode">
      <el-table-column prop="title" label="工具信息" min-width="200">
      ...
      <el-table-column prop="sceneDesc" label="类型" width="220" />
      ...
      <el-table-column label="操作" align="left" width="55">
        <template #default="scope">
          <KeenActions
...
<script lang="ts" setup name="LuteAssistantUtilConfig">
  ...
  const handleAdd = () => {
    router.push({ name: 'LuteAssistantUtilConfigDetail' });
  };
  ...
  const handleActions = (item) => {
    ...
    if (key === 'view') {
      router.push({ name: 'LuteAssistantUtilConfigDetail', query: { code } });
    }
  };
