<template>
  ...
  >
    ...
    >
      ...
      <el-table-column min-width="160px" prop="name" label="预算表" />
      <el-table-column min-width="160px" prop="budgetPeriod" label="预算期间" />
      <el-table-column min-width="100px" prop="createTime" label="创建时间" />
      <el-table-column min-width="70px" prop="distributorName" label="分发人" />
      <el-table-column min-width="100px" prop="budgetDeptName" label="所属部门" />
      ...
      <el-table-column prop="presentationDrafts" label="执行人一稿上报截止时间" min-width="170px">
        <template #default="{ row }">
          <div v-if="row?.presentationDrafts">
            {{ row?.presentationDrafts?.[0]?.draftDeadline }}
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="presentationDrafts" label="一稿修订版上报截止时间" min-width="170px">
        <template #default="{ row }">
          <div v-if="row?.presentationDrafts">
            {{ row?.presentationDrafts?.[1]?.draftDeadline }}
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="presentationDrafts" label="终稿上报截止时间" min-width="130px">
        <template #default="{ row }">
          <div v-if="row?.presentationDrafts">
            {{ row?.presentationDrafts?.[2]?.draftDeadline }}
          </div>
        </template>
      </el-table-column>
...
<script lang="ts" setup name="budgetCompilationPlatform">
  ...
  >({
    searchDefaults: {
      requestEntrance: 2, //请求入口 1-预算编制 2-预算编制台
  ...
  const handleActions = (item) => {
    const { key, row } = item;
    if (key === 'file') {
      fileModelRef.value.open('list', row.code);
    } else if (key === 'edit') {
      openPage(row, row.nowStepCode - 1);
    }
  };
  const selectedCodeList = ref([]);
  const handleSelectionChange = async (list) => {
    if (list) {
      selectedCodeList.value = list.map((res) => {
        return res.code;
      });
    } else {
  ...
  const openBudgetCompilationPlatform = (row, type) => {
  ...
  const openPage = (row, type) => {
    router.push({
      ...
      query: {
        ...
        planYear: row.planYear || row.budgetYear,
