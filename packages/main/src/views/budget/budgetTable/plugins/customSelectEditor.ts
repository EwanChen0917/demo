...
import { CellType, TableColumn } from '@/views/budget/budgetTable/types/type';

export class CustomSelectEditor extends Handsontable.editors.BaseEditor {
  // 注册插件时初始化
  init() {
    // console.log('in init');
    // 创建下拉框容器
    ...
    this.container.className = 'budgetSelect';
    ...
    TableEventBus.on('select-cell', ({ rowStart, colStart, rowEnd, colEnd }) => {
      // console.log(this.row, this.col, rowStart, colStart);
      if (this.row !== rowStart || this.col !== colStart) {
        if (this._opened) {
          const value = this.getFilterValue().toLowerCase();
          // console.log('in set value', value);
          const { allOptions } = this;
          const item = allOptions.find((item) => item.toLowerCase() === value);
          this.finish(this.row, this.col, item);
        }
      }
    });
  }
  ...
  prepare(row, col, prop, td, originalValue, cellProperties) {
    ...
    } else if (cellType === CellType.SelectCascade) {
      if (parentColumn) {
  ...
  finish(row, col, value) {
    // console.trace();
    // console.log('in finish',row,col,value);
    ...
    const { columnMeta } = this.cellProperties;
    const changes = [[this.row, columnMeta.cellFieldName, value]];

    // 级联联动
    if (columnMeta.cellType === CellType.SelectCascade) {
  ...
  handleSelectChange(ev) {
    ...
    this.finish(this.row, this.col, value);
  }
  ...
  // 过滤选项（防抖处理）
  filterOptions() {
    const filterText = this.getFilterValue().toLowerCase();
    ...
    if (!filterText) {
      ...
      if (filterText.includes('*')) {
        const reg = buildFuzzySearchText(filterText, 'i');
  ...
  loadMoreOptions() {
    ...
    chunk.forEach((item) => {
      ...
      optionElement.setAttribute('data-value', item);
      if (item === this._openInitValue) {
        optionElement.classList.add('selected');
      }
      optionElement.title = item;
      if (filterReg) {
        optionElement.innerHTML = item.replace(
          filterReg,
...
Handsontable.editors.CustomSelectEditor = CustomSelectEditor;
Handsontable.editors.registerEditor(CustomSelectEditor);
