...
export class CustomPercentEditor extends Handsontable.editors.BaseEditor {
  // 初始化编辑器
  init() {
    // 创建编辑器容器
  ...
  getValue() {
    ...
    if (isFinite(+value)) {
      return Decimal.div(+value, 100).toNumber();
    }
    return value;
  }
  ...
  __setValue(value) {
    ...
    this.input.value = isFinite(+num) ? Decimal.mul(+num, 100).toNumber() : '';
  }
...
// 注册编辑器
Handsontable.editors.CustomPercentEditor = CustomPercentEditor;
Handsontable.editors.registerEditor(CustomPercentEditor);
