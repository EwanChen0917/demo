          ...
          <div class="line-content" @click="selectLine(line)">
            <svg overflow="visible" width="20" height="20">
              ...
              <path
                ...
                :marker-start="
                  line.points[0] ? `url(#${`preset-line-${i}-${j}`}-${line.points[0]}-start)` : ''
                "
                :marker-end="
                  line.points[1] ? `url(#${`preset-line-${i}-${j}`}-${line.points[1]}-end)` : ''
                "
              />
            </svg>
          </div>
...
<script lang="ts" setup>
  import { LINE_LIST, type LinePoolItem } from '@/components/PPTist/configs/lines';

  import LinePointMarker from '@/components/PPTist/components/element/LineElement/LinePointMarker.vue';
