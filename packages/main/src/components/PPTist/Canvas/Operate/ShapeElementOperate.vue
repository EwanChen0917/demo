...
<script lang="ts" setup>
  ...
  import type { PPTShapeElement } from '@/components/PPTist/types/slides';
  import type { OperateResizeHandlers } from '@/components/PPTist/types/edit';
  import { SHAPE_PATH_FORMULAS } from '@/components/PPTist/configs/shapes';
  ...
  const keypoints = computed(() => {
    ...
    return props.elementInfo.keypoints.map((keypoint, index) => {
      ...
      if (relative === 'left') styles = { left: `${keypointPos * canvasScale.value}px` };
        ...
        styles = { left: `${(props.elementInfo.width - keypointPos) * canvasScale.value}px` };
      else if (relative === 'center')
        styles = { left: `${((props.elementInfo.width - keypointPos) / 2) * canvasScale.value}px` };
      else if (relative === 'top') styles = { top: `${keypointPos * canvasScale.value}px` };
      else if (relative === 'bottom')
        styles = { top: `${(props.elementInfo.height - keypointPos) * canvasScale.value}px` };
        ...
        styles = {
          left: `${(props.elementInfo.width - keypointPos) * canvasScale.value}px`,
          top: `${props.elementInfo.height * canvasScale.value}px`,
        };
        ...
        styles = {
          left: `${props.elementInfo.width * canvasScale.value}px`,
          top: `${(props.elementInfo.height - keypointPos) * canvasScale.value}px`,
        };
