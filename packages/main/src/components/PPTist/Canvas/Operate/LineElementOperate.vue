...
<script lang="ts" setup>
  ...
  import type { PPTLineElement } from '@/components/PPTist/types/slides';
  import { OperateLineHandlers } from '@/components/PPTist/types/edit';
  ...
  const resizeHandlers = computed(() => {
    ...
    if (props.elementInfo.curve || props.elementInfo.broken || props.elementInfo.broken2) {
      const ctrlHandler = (props.elementInfo.curve ||
        props.elementInfo.broken ||
        props.elementInfo.broken2) as [number, number];
