...
} from '@/components/PPTist/types/edit';
...
import { type AlignLine, uniqAlignLines } from '@/components/PPTist/utils/element';
import { MIN_SIZE } from '@/components/PPTist/configs/element';
...
export default (
  ...
  const scaleElement = (
    ...
    const handleMousemove = (e: MouseEvent | TouchEvent) => {
      ...
      if (elRotate) {
        const revisedX =
          (Math.cos(rotateRadian) * x + Math.sin(rotateRadian) * y) / canvasScale.value;
        let revisedY =
          (Math.cos(rotateRadian) * y - Math.sin(rotateRadian) * x) / canvasScale.value;
