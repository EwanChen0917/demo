  ...
  <div class="theme-styles-extract">
    ...
    <div class="content">
      <div class="config-item" v-if="themeStyles.fontNames.length">
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontNames" :key="item">
            ...
            <div class="handler">
              <div class="state" :class="{ active: selectedIndex.fontName === index }">
                <IconCheck />
              </div>
      ...
      <div class="config-item" v-if="themeStyles.fontColors.length">
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.fontColors" :key="item">
            <div
              class="value"
              :style="{ backgroundColor: item, color: getMostReadableColor(item) }"
            >
              {{ getHexColor(item) }}
            </div>
            <div class="handler">
              <div class="state" :class="{ active: selectedIndex.fontColor === index }">
                <IconCheck />
              </div>
      ...
      <div class="config-item" v-if="themeStyles.backgroundColors.length">
        ...
        <div class="values">
          <div class="value-wrap" v-for="(item, index) in themeStyles.backgroundColors" :key="item">
            ...
            <div class="handler">
              <div class="state" :class="{ active: selectedIndex.backgroundColor === index }">
                <IconCheck />
              </div>
      ...
      <div class="config-item" v-if="themeStyles.themeColors.length">
        <div class="label">
          主题色：
          <span class="tip">（点击色块排除不要的颜色）</span>
        </div>
        <div class="values inline">
          <div
            ...
            :key="item"
            @click="removeThemeColor(index)"
          >
            <div
              class="value"
              :class="{ disabled: !selectedIndex.themeColors.includes(index) }"
...
<script lang="ts" setup>
  ...
  import type { SlideTheme } from '@/components/PPTist/types/slides';
  import message from '@/components/PPTist/utils/message';
  import { FONTS } from '@/components/PPTist/configs/font';
  import Tabs from '@/components/PPTist/components/Tabs.vue';
  ...
  const updateAllThemes = () => {
    let themeColors = themeStyles.value.themeColors.filter((item, index) =>
      selectedIndex.value.themeColors.includes(index)
    );
