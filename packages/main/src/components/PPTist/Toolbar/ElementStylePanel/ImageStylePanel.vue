<template>
  <div class="image-style-panel">
    <div class="origin-image" :style="{ backgroundImage: `url(${handleImageElement.src})` }"></div>
...
<script lang="ts" setup>
  ...
  import type { PPTImageElement, SlideBackground } from '@/components/PPTist/types/slides';
  import { getImageDataURL, getImageSize } from '@/components/PPTist/utils/image';
  import { CLIPPATHS } from '@/components/PPTist/configs/imageClip';
  ...
  const getImageElementDataBeforeClip = () => {
    ...
    const originClipRange: [[number, number], [number, number]] = _handleElement.clip
