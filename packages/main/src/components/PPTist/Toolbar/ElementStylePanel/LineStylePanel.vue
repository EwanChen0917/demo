<template>
  <div class="line-style-panel">
      ...
      <Popover trigger="click" style="width: 60%">
        <template #content>
          <ColorPicker
            ...
            @update:model-value="(value) => updateLine({ color: value })"
          />
        </template>
    ...
    <div class="row">
      ...
      <SelectCustom style="width: 60%">
        <template #options>
          <div
            class="option"
            v-for="item in lineMarkerOptions"
            :key="item"
            @click="updateLine({ points: [item, handleLineElement.points[1]] })"
          >
      ...
      <SelectCustom style="width: 60%">
        <template #options>
          <div
            ...
            v-for="item in lineMarkerOptions"
            :key="item"
            @click="updateLine({ points: [handleLineElement.points[0], item] })"
          >
            <SVGLine :padding="5" :markers="['', item]" />
          </div>
        </template>
    ...
    <div class="row">
      <Button
        style="flex: 1"
        @click="updateLine({ start: handleLineElement.end, end: handleLineElement.start })"
      >
        <IconSwitch />
        交换方向
      </Button>
...
<script lang="ts" setup>
  ...
  import type { LinePoint, LineStyleType, PPTLineElement } from '@/components/PPTist/types/slides';
