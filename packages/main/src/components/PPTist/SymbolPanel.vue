<template>
  ...
  >
    ...
    <Tabs :tabs="tabs" v-model:value="selectedSymbolKey" :tabsStyle="{ marginBottom: '8px' }" />
...
<script lang="ts" setup>
  ...
  import { SYMBOL_LIST } from '@/components/PPTist/configs/symbol';
  import MoveablePanel from '@/components/PPTist/components/MoveablePanel.vue';
  ...
  const selectSymbol = (value: string) => {
    ...
    if (handleElement.value?.type === 'shape') {
      const editableElRef = document.querySelector(
        `#editable-element-${handleElement.value.id} .ProseMirror`
      );
      if (editableElRef) {
        emitter.emit(EmitterEvents.RICH_TEXT_COMMAND, { action: { command: 'insert', value } });
        return;
    ...
    if (handleElement.value?.type === 'table') {
      const editableElRef = document.querySelector(
        `#editable-element-${handleElement.value.id} .cell.active .cell-text`
      );
      if (editableElRef) {
