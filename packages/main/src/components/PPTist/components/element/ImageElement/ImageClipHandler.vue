<template>
  <div class="image-clip-handler" :style="clipWrapperPositionStyle" v-click-outside="handleClip">
    <img class="bottom-img" :src="src" :draggable="false" alt="" :style="bottomImgPositionStyle" />
    ...
    >
      <img class="top-img" :src="src" :draggable="false" alt="" :style="topImgPositionStyle" />
    </div>
    ...
    >
      ...
      >
        <svg width="16" height="16" fill="#fff" stroke="#333">
          <path stroke-width="0.3" shape-rendering="crispEdges" d="M 16 0 L 0 0 L 0 4 L 16 4 Z" />
        </svg>
      </div>
    </div>
  </div>
...
<script lang="ts" setup>
  ...
  import { useMainStore, useKeyboardStore } from '@/store/modules';
  import type { ImageClipDataRange, ImageElementClip } from '@/components/PPTist/types/slides';
  import { KEYS } from '@/components/PPTist/configs/hotkey';
  import { type ImageClipedEmitData, OperateResizeHandlers } from '@/components/PPTist/types/edit';
  ...
  const scaleClipRange = (e: MouseEvent, type: OperateResizeHandlers) => {
    ...
    document.onmousemove = (e) => {
      ...
      if (ctrlOrShiftKeyActive.value) {
        if (type === OperateResizeHandlers.RIGHT_BOTTOM || type === OperateResizeHandlers.LEFT_TOP)
          moveY = moveX / aspectRatio;
        if (type === OperateResizeHandlers.LEFT_BOTTOM || type === OperateResizeHandlers.RIGHT_TOP)
          moveY = -moveX / aspectRatio;
      }
