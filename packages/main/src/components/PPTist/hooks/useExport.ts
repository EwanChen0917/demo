...
} from '@/components/PPTist/utils/element';
import { type AST, toAST } from '@/components/PPTist/utils/htmlParser';
import { type SvgPoints, toPoints } from '@/components/PPTist/utils/svgPathParser';
import { encrypt } from '@/components/PPTist/utils/crypto';
import { svg2Base64 } from '@/components/PPTist/utils/svg2Base64';
import message from '@/components/PPTist/utils/message';
...
export default () => {
  ...
  const isSVGImage = (url: string) => {
    const isSVGBase64 = url.startsWith('data:image/svg+xml;base64,');
  ...
  // 将任意图片Blob绘制到Canvas并导出为PNG，以规避PPT对部分格式（如webp）的兼容问题
  ...
  // 远程图片转DataURL（含SVG/WEBP处理）
  const urlToDataURL = async (url: string): Promise<string | null> => {
    try {
      if (isBase64Image(url)) return url;
      if (isSVGImage(url)) {
        // 将SVG统一转成PNG，避免PPT对SVG不渲染的问题
        if (/^data:/.test(url)) {
          // data:svg 也栅格化
          ...
          if (!res.ok) {
            console.warn(`[PPT导出] 图片加载失败(HTTP ${res.status})，已跳过: ${url}`);
            return null;
          }
          const text = await res.text();
          const svgBlob = new Blob([text], { type: 'image/svg+xml' });
          return await blobToPngDataURL(svgBlob);
        }
      }
      if (/^https?:\/\//.test(url)) {
        const res = await fetch(url, { mode: 'cors', credentials: 'omit' });
      ...
      // CORS 或网络错误时记录警告并跳过
      console.warn(`[PPT导出] 图片加载失败(可能是CORS问题)，已跳过: ${url}`, e);
      return null;
    }
  };
  ...
  const exportPPTX = async (_slides: Slide[], masterOverwrite: boolean, ignoreMedia: boolean) => {
    ...
    // 预取远程图片为dataURL，避免跨域及PPT不支持的格式
    const urlSet = new Set<string>();
    for (const s of _slides) {
      if (s.background?.type === 'image' && s.background.image?.src) {
        const src = s.background.image.src;
        if (!isBase64Image(src)) urlSet.add(src);
      }
      if (!s.elements) continue;
      for (const el of s.elements) {
        if (el.type === 'image') {
          if (el.src && !isBase64Image(el.src)) urlSet.add(el.src);
        } else if (el.type === 'shape') {
          if (el.pattern && !isBase64Image(el.pattern)) urlSet.add(el.pattern);
        } else if (!ignoreMedia && el.type === 'video') {
          if (el.poster && !isBase64Image(el.poster)) urlSet.add(el.poster);
        }
      }
    }
    ...
    if (urlArr.length) {
      const results = await Promise.all(
        urlArr.map(async (u) => ({ u, data: await urlToDataURL(u) }))
      );
      for (const { u, data } of results) if (data) dataUrlMap.set(u, data);
    }
    ...
    for (const slide of _slides) {
      ...
      if (slide.background) {
        ...
        if (background.type === 'image' && background.image) {
          const bgSrc = background.image.src;
          const mapped = dataUrlMap.get(bgSrc);
          if (isSVGImage(bgSrc) || isBase64Image(bgSrc) || mapped) {
            pptxSlide.addImage({
              data: mapped || bgSrc,
              x: 0,
              y: 0,
            ...
            // 远程图片无法获取时跳过，避免 CORS 问题导致导出失败
            console.warn(`[PPT导出] 背景图片无法加载，已跳过: ${bgSrc}`);
          }
        } else if (background.type === 'solid' && background.color) {
          const c = formatColor(background.color);
          pptxSlide.background = { color: c.color, transparency: (1 - c.alpha) * 100 };
        } else if (background.type === 'gradient' && background.gradient) {
          const { colors } = background.gradient;
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          const mapped = dataUrlMap.get(el.src);
          // 检查图片是否可用：必须是 base64 或已成功预加载的远程图片
          if (!isBase64Image(el.src) && !mapped) {
            // 远程图片无法获取时跳过，避免 CORS 问题导致导出失败
            console.warn(`[PPT导出] 图片无法加载，已跳过: ${el.src}`);
            continue;
          }
          ...
          if (el.pattern) {
            const mappedPattern = dataUrlMap.get(el.pattern);
            // 检查 pattern 图片是否可用
            if (!isBase64Image(el.pattern) && !mappedPattern) {
              // 远程 pattern 图片无法获取时跳过
              console.warn(`[PPT导出] Shape pattern 图片无法加载，已跳过: ${el.pattern}`);
            } else {
              const options: pptxgen.ImageProps = {
                x: el.left / ratioPx2Inch.value,
                y: el.top / ratioPx2Inch.value,
          ...
          for (let i = 0; i < el.data.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
              ...
              const cellOptions: pptxgen.TableCellProps = {
                ...
                fontSize:
                  (cell.style?.fontsize ? parseInt(cell.style?.fontsize) : 14) / ratioPx2Pt.value,
              };
              if (theme && themeColor) {
                let c: FormatColor;
          ...
          if (el.type === 'video' && el.poster) {
            const mappedPoster = dataUrlMap.get(el.poster);
            options.cover = mappedPoster || el.poster;
          }
