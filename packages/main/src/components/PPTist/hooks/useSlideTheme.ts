...
import type { PresetTheme } from '@/components/PPTist/configs/theme';
import { getLineElementLength } from '@/components/PPTist/utils/element';
...
export default () => {
    ...
    for (const slide of slides) {
      ...
      for (const el of slide.elements) {
        ...
        if (el.type === 'line') {
          ...
          elHeight = Math.sqrt(Math.abs(startX - endX) ** 2 + Math.abs(startY - endY) ** 2);
        ...
        if (el.type === 'shape' || el.type === 'text') {
          ...
          if (defaultFontPercent) {
            const _defaultFont =
              el.type === 'shape' ? el.text?.defaultFontName : el.defaultFontName;
  ...
  // 创建原颜色与新颜色的对应关系表
  const createSlideThemeColorMap = (
    slide: Slide,
    _newColors: string[]
  ): { [key: string]: string } => {
