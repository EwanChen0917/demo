...
import type { AIPPTSlide } from '@/components/PPTist/types/AIPPT';
...
export default () => {
  ...
  const getAdaptedFontsize = ({
    ...
    while (newFontSize >= minFontSize) {
      ...
      if (maxLine > 1 && height) {
        const heightOfLine =
          Math.max(newFontSize, 16) * (newFontSize < 15 ? 1.2 : lineHeight) * 1.2;
  ...
  const AIPPT = (templateSlides: Slide[], _AISlides: AIPPTSlide[], imgs?: ImgPoolItem[]) => {
    ...
    for (const item of AISlides) {
      if (item.type === 'cover') {
        ...
        const _contentsTemplates = getUseableTemplates(
          contentsTemplates,
          item.data.items.length,
          'item'
        );
        const contentsTemplate =
          _contentsTemplates[Math.floor(Math.random() * _contentsTemplates.length)];

        const sortedNumberItems = contentsTemplate.elements.filter((el) =>
          checkTextType(el, 'itemNumber')
        );
        ...
        const elements = contentsTemplate.elements
          .map((el) => {
            ...
            if (checkTextType(el, 'item')) {
              ...
              if (itemTitle)
                return getNewTextElement({ el, text: itemTitle, maxLine: 1, longestText });
          ...
          .filter(
            (el) =>
              !unusedElIds.includes(el.id) && !(el.groupId && unusedGroupIds.includes(el.groupId))
          );
        slides.push({
          ...contentsTemplate,
          id: nanoid(10),
        ...
        const _contentTemplates = getUseableTemplates(
          contentTemplates,
          item.data.items.length,
          'item'
        );
        const contentTemplate =
          _contentTemplates[Math.floor(Math.random() * _contentTemplates.length)];
        ...
        const longestTitle = itemTitles.reduce(
          (longest, current) => (current.length > longest.length ? current : longest),
          ''
        );
        const longestText = itemTexts.reduce(
          (longest, current) => (current.length > longest.length ? current : longest),
          ''
        );
