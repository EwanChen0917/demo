...
import { ShapePathFormulasKeys } from '@/components/PPTist/types/slides';
...
export const SHAPE_PATH_FORMULAS: {
  ...
  [ShapePathFormulasKeys.ROUND_RECT]: {
    ...
    formula: (width, height, values) => {
      ...
      return `M ${radius} 0 L ${width - radius} 0 Q ${width} 0 ${width} ${radius} L ${width} ${
        height - radius
      } Q ${width} ${height} ${width - radius} ${height} L ${radius} ${height} Q 0 ${height} 0 ${
        height - radius
  ...
  [ShapePathFormulasKeys.ROUND_RECT_DIAGONAL]: {
    ...
    formula: (width, height, values) => {
      const radius = Math.min(width, height) * values![0];
      return `M ${radius} 0 L ${width} 0 L ${width} ${height - radius} Q ${width} ${height} ${
        width - radius
  ...
  [ShapePathFormulasKeys.MESSAGE]: {
    ...
    formula: (width, height, values) => {
      ...
      return `M 0 0 L ${width} 0 L ${width} ${height - arrowheight} L ${point + arrowWidth} ${
        height - arrowheight
  ...
  [ShapePathFormulasKeys.ROUND_MESSAGE]: {
    formula: (width, height) => {
      ...
      return `M 0 ${radius} Q 0 0 ${radius} 0 L ${
        width - radius
      } 0 Q ${width} 0 ${width} ${radius} L ${width} ${height - radius - arrowheight} Q ${width} ${
        height - arrowheight
      } ${width - radius} ${height - arrowheight} L ${width / 2} ${height - arrowheight} L ${
        width / 2 - arrowWidth
      } ${height} L ${width / 2 - arrowWidth} ${height - arrowheight} L ${radius} ${
        height - arrowheight
  ...
  [ShapePathFormulasKeys.PLUS]: {
    ...
    formula: (width, height, values) => {
      ...
      return `M ${width / 2 - lineWidth / 2} 0 L ${width / 2 - lineWidth / 2} ${
        ...
        width / 2 - lineWidth / 2
      } ${height / 2 + lineWidth / 2} L ${width / 2 - lineWidth / 2} ${height} L ${
        width / 2 + lineWidth / 2
      } ${height} L ${width / 2 + lineWidth / 2} ${height / 2 + lineWidth / 2} L ${width} ${
        height / 2 + lineWidth / 2
      } L ${width} ${height / 2 - lineWidth / 2} L ${width / 2 + lineWidth / 2} ${
        height / 2 - lineWidth / 2
      } L ${width / 2 + lineWidth / 2} 0 Z`;
    },
  },
